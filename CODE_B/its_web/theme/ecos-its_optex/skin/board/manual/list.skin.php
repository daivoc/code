...
// if($member['mb_level'] == 6) $iAmManager = 1;
...
function list_w_group_main() {
	...
	$list_w_group_id = '<a href='.$g5_url.'/bbs/board.php?bo_table='.$bo_table.' class="btn btn-sm btn-success">전체</a>';
	foreach ($manual_group as $key => $value) {
		$list_w_group_id .='<a href='.$g5_url.'/bbs/board.php?bo_table='.$bo_table.'&sca=&sop=and&sfl=w_group_main&stx='.$key.' class="btn btn-sm btn-default">'.$key.'</a>';
	}
...
<section id="bo_list" class="container">
	...
	<!-- 게시판 페이지 정보 및 버튼 시작 { -->
	<div class="bo_fx">
		<div id="bo_list_total" class="w_hide">
	...
	<div class="tbl_head01 tbl_wrap table-responsive">
		...
		<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
			...
			<td class=""><?php echo $list[$i]['w_sub_title'] ?></td>
			<td class=""><?php echo $list[$i]['w_group_main'] ?></td>
			<td class=""><?php echo $list[$i]['w_group_sub'] ?></td>
			<td class="td_num"><?php echo $list[$i]['w_order'] ?></td>
	...
	<div class="bo_fx">
		...
		<ul class="btn_bo_user">
			<?php if ($list_href) { ?><li><a href="<?php echo $list_href ?>" class="btn btn-sm btn-primary"><?php echo $SK_BO_List[ITS_Lang]?></a></li><?php } ?>
			<?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="btn btn-sm btn-primary"><?php echo $SK_BO_Write[ITS_Lang]?></a></li><?php } ?>
		</ul>
		<?php } ?>
	</div>
