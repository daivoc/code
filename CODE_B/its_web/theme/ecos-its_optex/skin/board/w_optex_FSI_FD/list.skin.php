...
// 목록 보기를 부모 목록으로 전환 한다.
header("Location: /bbs/board.php?bo_table=g400t100");
die();
...
$(document).ready(function(){
	$('.btn_Events').click(function(){
		...
		$('#TheForm input[name="w_device_serial"]').val("");
		...
		$('#TheForm input[name="w_device_serial"]').val($(this).data("serial"));
...
<section id="bo_list" class="container">
	...
	<div class="bo_fx">
		...
		<ul class="btn_bo_user">
			...
			<li><a href="/bbs/board.php?bo_table=g400t100" class="btn btn-sm btn-success f_right">FSI</a></li>
		</ul>
		<?php } ?>
	</div>
	<!-- } 게시판 페이지 정보 및 버튼 끝 -->
	...
	<div class="tbl_head01 tbl_wrap table-responsive">
		...
		<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
			...
			<td class="td_board"><?php echo $list[$i]['w_device_serial'] ?></td>
			<td class="td_board pointer" onclick='window.open("<?php echo G5_THEME_URL ?>/utility/scheduleBlock/weekView.php?bo_table=<?php echo $bo_table?>&wr_id=<?php echo $list[$i]['wr_id']?>&wr_subject=<?php echo $list[$i]['wr_subject']?>&w_device_serial=<?php echo $list[$i]['w_device_serial']?>","scheduleWeek_GPIO_<?php echo $list[$i]['wr_id']?>", "width=600,height=500,scrollbars=no");'><?php echo $SK_BO_Weekly[ITS_Lang]?></td>
			<td class="td_board pointer" onclick='window.open("<?php echo G5_THEME_URL ?>/utility/scheduleBlock/index.php?bo_table=<?php echo $bo_table?>&wr_id=<?php echo $list[$i]['wr_id']?>&wr_subject=<?php echo $list[$i]['wr_subject']?>&w_device_serial=<?php echo $list[$i]['w_device_serial']?>","scheduleBlock_GPIO_<?php echo $list[$i]['wr_id']?>", "width=600,height=500,scrollbars=no");'><?php echo $SK_BO_Daily[ITS_Lang]?></td>
			<!-- td class="td_board pointer" onclick='window.open("<?php echo G5_CU_UTIL_URL ?>/ecosFileTree/snapshotView.php?ID=<?php echo $list[$i]['w_device_serial']?>&wr_subject=<?php echo $list[$i]['wr_subject']?>","Shot_Tree_<?php echo $list[$i]['w_device_serial']?>", "width=800,height=400,scrollbars=no");' >Shot</td -->
			<td class="td_board pointer btn_Events" data-serial="<?php echo $list[$i]['w_device_serial']?>" data-subject="<?php echo $list[$i]['wr_subject']?>"><?php echo $SK_BO_Log[ITS_Lang]?></td>
