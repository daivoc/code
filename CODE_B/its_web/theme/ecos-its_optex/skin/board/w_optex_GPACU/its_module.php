...
function get_w_gpacu_serial($bo_table, $wr_id) { // MD5(Wits IP + GPACU ID) $w_system_ip, $w_device_id
	...
	// $get_w_gpacu_serial =  strtoupper($string);
	return $get_w_gpacu_serial;
}
...
function select_w_gpacu_status($cur_w_gpacu_status) {
    global $g5, $bo_table, $event_status;
	
	$select_w_gpacu_status = '<select name="w_gpacu_status" id="w_gpacu_status" class="form-control input25P required" required>';
	while (list($key, $value) = each($event_status)) { 
		if($cur_w_gpacu_status == $key)
			$select_w_gpacu_status .='<option selected value="'.$key.'">'.$value.'</option>';
		else
			$select_w_gpacu_status .='<option value="'.$key.'">'.$value.'</option>';
	} 
	$select_w_gpacu_status .= '</select>';
	return $select_w_gpacu_status;
}
