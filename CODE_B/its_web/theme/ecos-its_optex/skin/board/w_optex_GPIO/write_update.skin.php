...
$w_id = $w_cpu_id;
if($w_opt12) {
	$w_sensor_serial = get_w_sensor_serial($bo_table, $w_system_ip, "0000"); // MD5(Wits IP + '||' + Device ID)  
} else {
	$w_sensor_serial = get_w_sensor_serial($bo_table, $w_system_ip, $wr_id); // MD5(Wits IP + '||' + Device ID)  
}
$w_table_PortIn = getNodePort($cfg["init_value"]["DEVICE_table_PortIn"], $bo_table, $wr_id, $w_device_id);
$w_table_PortOut = getNodePort($cfg["init_value"]["DEVICE_table_PortOut"], $bo_table, $wr_id, $w_device_id);
