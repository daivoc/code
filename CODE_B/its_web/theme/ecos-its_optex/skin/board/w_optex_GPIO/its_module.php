...
function get_w_sensor_Addr($cur_w_sensor_Addr) {
	if($cur_w_sensor_Addr) {
		...
		$get_w_sensor_Addr = "192.168.168.30";
	}
...
// GPIO 디바이스포트, 템퍼포트는 디바이스포트의 짝수로 한다.
function select_w_GPIO_id($cur_w_device_id) {
    global $g5, $bo_table, $cfg;
	$write_table = $g5['write_prefix'] . $bo_table;
	$sql = " SELECT * FROM $write_table WHERE w_sensor_disable = 0 ";
    $result = sql_query($sql);
	...
	$select_w_device_id = '<select name="w_device_id" id="w_device_id" class="form-control input50P" placeholder="<?php echo $SK_BO_Link_Interface[ITS_Lang]?>" ><option value="" disabled selected>Device ID</option>';
	while (list($key, $value) = each($cfg["DEVICE_all"])) { 
		if($cur_w_device_id == $key) {
			$select_w_device_id .='<option selected value="'.$key.'">'.$value.'</option>';
		} else {
			if(in_array($key, $usedKey)) { // 다른 센서가 사용중이면 중복사용 금지
...
function select_w_sensor_model($cur_w_sensor_model) {
	$arr = array('RELAY');
	$select_w_sensor_model = '<select name="w_sensor_model" id="w_sensor_model" required class="form-control input50P required" placeholder="<?php echo $SK_BO_Model_Name[ITS_Lang]?>" >';
