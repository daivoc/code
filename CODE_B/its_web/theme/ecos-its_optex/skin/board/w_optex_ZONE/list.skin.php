...
// 그룹 목록 배열 재 생성
$groupData = array(); 
// 데이터 예: g300t500_192_168_0_14_0003 g300t500_192_168_0_14_0002 g300t100_192_168_0_14_0003
for ($i=0; $i<count($list); $i++) { 
	// echo $list[$i]['w_sns_0'];
	$zoneID = substr(substr($list[$i]['w_sns_0'], 9), 0, -5); // 192_168_0_14
	$groupData[$zoneID][$list[$i]['wr_id']] = $list[$i];
}
// $output = json_encode($groupData, JSON_PRETTY_PRINT); // Pretty-Printing JSON with PHP
...
// 		$acdnListName .= "{$value_s['wr_subject']} ";
// 		$acdnListVal .= "<p class='listVal'>{$value_s['wr_subject']}</p>";
// 	}
// 	$acdnList .= "<details class='acdn'>";
// 	// echo "{$key} => {$value} ";
// 	$acdnList .= "<summary class='acdn-header'><div class='listGroup'>{$key}</div> <div class='listName'>{$acdnListName}</div></summary>";
// 	$acdnList .= "<div class='acdn-body'>";
// 	$acdnList .= $acdnListVal;
// 	$acdnList .= "</div>";
// 	$acdnList .= "</details>";
// }
...
foreach ($groupData as $key => $value) {
	...
	foreach ($value as $key_s => $value_s) {
		$acdnListID += 1;
		if ($value_s['w_zone_disable'] == 1) {
			$cur_subject = "<del>{$value_s['subject']}</del>";
		} else {
			$cur_subject = "{$value_s['subject']}";
		}
		$acdnListName .= "<div class='listName'><a href={$value_s['href']}>{$cur_subject}</a></div> ";
		$acdnListVal .= "<tr><td class='board_CU acdnTD'>{$acdnListID}</td><td class='board_CU acdnTD'><a href={$value_s['href']}>{$cur_subject}</a></td><td class='board_CU acdnTD'>{$value_s['w_zone_model']}</td><td class='board_CU acdnTD' style='cursor:pointer;color:green;' onclick=\"window.open('{$mapIMS}{$value_s['w_map_id']}','mapIMS', 'width=600,height=400,scrollbars=no');\">{$value_s['w_map_id']}</td><td class='board_CU acdnTD'>".get_w_camInfo($value_s['w_cam_0'], 'wr_subject')."</td><td class='board_CU acdnTD'>{$value_s['w_sns_0']}</td><td class='board_CU acdnTD'>{$value_s['w_zone_serial']}</td><td class='board_CU acdnTD'></td></tr>";
	}

	$acdnList .= "<details class='acdn'>";
	// echo "{$key} => {$value} ";
	$acdnList .= "<summary class='acdn-header'><div class='listGroup' style='display:list-item;background: white;color: #486583;'>{$acdnGropID}</div> <div class='listGroup'>{$key}</div> {$acdnListName}</summary>";
	$acdnList .= "<table class='table-hover' style='margin: 2px 0px 10px 4%; width: 96%;'><tr><tr><th scope='col' class='acdnTH'>No.</th><th scope='col' class='acdnTH'>디바이스</th><th scope='col' class='acdnTH'>모델명</th><th scope='col' class='acdnTH'>맵아이디</th><th scope='col' class='acdnTH'>켐아이디</th><th scope='col' class='acdnTH'>센서아이디</th><th scope='col' class='acdnTH'>존아이디</th></tr>";
	$acdnList .= $acdnListVal;
	$acdnList .= "</table>";
	$acdnList .= "</details>";
}
...
<section id="bo_list" class="container">
	...
	<div class="bo_fx">
		...
		<ul class="btn_bo_user">
			...
			<li><a href="/bbs/board.php?bo_table=g500t300" class="btn btn-sm btn-success f_right"><?php echo $SK_BO_Box[ITS_Lang]?></a></li>
		</ul>
		<?php } ?>
	</div>
	<!-- } 게시판 페이지 정보 및 버튼 끝 -->
	...
	<form class='w_hide' name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
	...
	<div class="tbl_head01 tbl_wrap table-responsive">
		...
		<tr>
			...
			<th scope="col"><?php echo $SK_BO_Model_Name[ITS_Lang]?></th>
			<th scope="col"><?php echo $SK_BO_Map[ITS_Lang].$SK_BO_ID[ITS_Lang]?></th>
			<th scope="col"><?php echo $SK_BO_Sensor[ITS_Lang].$SK_BO_ID[ITS_Lang]?></th>
		...
		<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
			...
			<td class="board_CU"><?php echo $list[$i]['w_zone_model'] ?></td>
			...
			<td class="board_CU"><?php echo $list[$i]['w_sns_0'] ?></td>
			<script>
