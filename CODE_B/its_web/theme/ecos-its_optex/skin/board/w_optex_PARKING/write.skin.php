...
if($w == '') {
	...
	$write['w_sensor_ignoreE'] = 0; // 적용범위 float NOT NULL DEFAULT '0',
	...
	$write['w_sensor_Addr'] = $SENSOR_IP_1; // 즉시적용 TINYINT(1) NOT NULL DEFAULT '0',
	$write['w_sensor_Port'] = $SENSOR_port_1; // 즉시적용 TINYINT(1) NOT NULL DEFAULT '0',
	$write['w_sensor_Addr2'] = $SENSOR_IP_2; // 즉시적용 TINYINT(1) NOT NULL DEFAULT '0',
	$write['w_sensor_Port2'] = $SENSOR_port_2; // 즉시적용 TINYINT(1) NOT NULL DEFAULT '0',
	...
	$write['w_alarm_level'] = 1; // 알람등급 tinyint(4) NOT NULL DEFAULT '0',
}
?>
...
#select_w_sensor_ignoreZone .ui-selecting { background: gray; }
#select_w_sensor_ignoreZone .ui-selected { background: #2196f3; color: white; }
#select_w_sensor_ignoreZone { list-style-type: none; margin: 0; padding: 0; width: 100%; }
#select_w_sensor_ignoreZone li { margin: 1px; padding: 2px; float: left; width: 12%; height: 34px; border-radius: 18px; font-size: 12pt; text-align: center; }
#select_w_sensor_scheduleZone .ui-selecting { background: gray; }
#select_w_sensor_scheduleZone .ui-selected { background: #2196f3; color: white; }
#select_w_sensor_scheduleZone { list-style-type: none; margin: 0; padding: 0; width: 100%; }
#select_w_sensor_scheduleZone li { margin: 1px; padding: 2px; float: left; width: 12%; height: 34px; border-radius: 18px; font-size: 12pt; text-align: center; }
...
$(document).ready(function(){
	...
	$('#w_event_pickTime').prop('readonly', true);
	$('#w_sensor_week').prop('readonly', true);
...
<section id="bo_w" class="container">
    ...
    <div class="tbl_wrap table-responsive">
		...
		<tbody>
		<tr>
			<th scope="row" title="Title"><label for="wr_subject">Location/Gate<strong class="sound_only">필수</strong></label></th>
			<td>
		...
		<tr class="w_hide">
			<th scope="row" title="USB Device ID"><label for="w_device_id">Model/Serial</label></th>
			<td>
		...
		<tbody>
		<tr class="w_hide">
			<th scope="row" title="Accept Range"><label for="w_sensor_ignore">차단지역</label></th>
			<td>
				<?php echo select_w_sensor_ignoreZone($write['w_sensor_ignoreZone'], $MAX_numberOfZone); ?>
			</td>
		</tr>
		<tr class="w_hide">
			<th scope="row" title="Limit Due and Level"><label for="w_sensor_angle"><?php echo $SK_BO_Allowable[ITS_Lang]?></label></th>
			<td>
				<?php echo select_w_alarm_level($write['w_alarm_level']); ?>
				<div class="input25P" style="padding: 10px 15px;">
		...
		<tr class="w_default_tr">
			<th scope="row" title="Sensor Direction"><label for="w_sensor_face">Direction/Capacity<br>Max/Current</label></th>
			<td>
				<?php echo select_w_sensor_face($write['w_sensor_face']); ?>
				<input type="text" name="w_capacity_max" value="<?php echo $write['w_capacity_max'] ?>" id="w_capacity_max" class="form-control input25P" placeholder="Max Capacity" size="50">
				<input type="text" name="w_capacity_cur" value="<?php echo $write['w_capacity_cur'] ?>" id="w_capacity_cur" class="form-control input25P" placeholder="Current" size="50">
			</td>
		</tr>
		<tr class="w_hide">
			<th scope="row"><label for="w_sensor_angle">Angle</label></th>
			<td><input type="text" name="w_sensor_angle" value="<?php echo $write['w_sensor_angle'] ?>" id="w_sensor_angle" class="form-control" placeholder="Angle" size="50"></td>
		</tr>
		<tr class="w_hide">
		...
		<tr class="w_hide">
			<th scope="row" title="ITS Before After"><label for="w_systemBF_ip">ITS Before/After<br>IP/Port</label></th>
			<td><input type="text" name="w_systemBF_ip" value="<?php echo ($write['w_systemBF_ip']) ?>" id="w_systemBF_ip" class="form-control input25P" placeholder="System IP Address" size="50"><input type="text" name="w_systemBF_port" value="<?php echo $write['w_systemBF_port'] ?>" id="w_systemBF_port" class="form-control input25P" placeholder="Port Ex:50007" size="50"><input type="text" name="w_systemAF_ip" value="<?php echo ($write['w_systemAF_ip']) ?>" id="w_systemAF_ip" class="form-control input25P" placeholder="System IP Address" size="50"><input type="text" name="w_systemAF_port" value="<?php echo $write['w_systemAF_port'] ?>" id="w_systemAF_port" class="form-control input25P" placeholder="Port Ex:50007" size="50"></td>
		</tr>
		...
		<tr class="w_hide">
			<th scope="row"><label for="w_sensor_Addr2">Sensor2</label></th>
			<td><input type="text" name="w_sensor_Addr2" value="<?php echo get_w_sensor_Addr($write['w_sensor_Addr2']) ?>" id="w_sensor_Addr2" class="form-control input50P" placeholder="IP Ex:121.165.208.119" size="50"><input type="text" name="w_sensor_Port2" value="<?php echo get_w_sensor_Port($write['w_sensor_Port2']) ?>" id="w_sensor_Port2" class="form-control input50P" placeholder="Port Ex:50007" size="50"></td>
		</tr>
		<tr class="w_hide">
		...
		<tr class="w_detail_tr">
			...
			<td><input type="text" name="w_url1" value="<?php echo $write['w_url1'] ?>" id="w_url1" class="form-control" placeholder="URL Ex:http://user:pass@ip/cgi-bin/snapshot.cgi" size="50"></td>
		</tr>
		<tr class="w_hide">
		...
		<tr class="w_detail_tr">
			<th scope="row"><label for="w_neighbors_ip">Neighbors IP</label></th>
			<td><input type="text" name="w_neighbors_ip" value="<?php echo $write['w_neighbors_ip'] ?>" id="w_neighbors_ip" class="form-control" placeholder="Neighbors Max(10) IP:PORT{SPACE}" size="50"></td>
		</tr>
		<tr class="w_detail_tr">
			<th scope="row"><label for="w_alert_Port">Alert<br>ID/Second</label></th>
			<td>
		...
		<tr class="w_detail_tr">
			...
			<td><input type="text" name="w_keycode" value="<?php echo $write['w_keycode'] ?>" id="w_keycode" class="form-control input50P" placeholder="Key" size="50"><input type="text" name="w_license" value="<?php echo $write['w_license'] ?>" id="w_license" class="form-control input50P" placeholder="License" size="50"></td>
		</tr>
		...
		<tr>
			...
			<td class="w_hide">허용주기고정<input type="checkbox" class="form-control" name="w_event_keepHole" id="w_event_keepHole" value="1" <?php echo $write[w_event_keepHole]?'checked':'';?> title="Keep event hold cycle."></td>
			<td class="w_hide">동일위치고정<input type="checkbox" class="form-control" name="w_event_syncDist" id="w_event_syncDist" value="1" <?php echo $write[w_event_syncDist]?'checked':'';?> title="Allow event that same distence."></td>
		</tr>
		</tbody>
		</table>
		<table class="table table-bordered">
