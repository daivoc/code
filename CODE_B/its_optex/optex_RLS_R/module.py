...
def kill_demon_check_RLS_R(): 
	cmd = "pkill -9 -ef optex_RLS_R.pyc 2>&1" 
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
...
# 확인된 변수로 데몬을 실행 한다
def run_demon_check_RLS_R(arg): # python -W ignore
	cmd = "python /home/pi/optex_RLS_R/optex_RLS_R.pyc %s 2>&1 & " % arg
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)

	# ## ## optex_RLS_alarm 동시 실행
	# cmd = "python /home/pi/optex_RLS_R/optex_RLS_alarm.pyc %s 2>&1 & " % arg
	# p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)

# 실행하고 있는 arg 단어가 포함된 데몬을 awk를 이용하여 모두 종료 시킨다.
# def kill_demon_realtime_RLS_R(arg): 
	# cmd = "kill $(ps aux | grep '[n]ode realtime_RLS.js %s' | awk '{print $2}')" % arg
	# p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	# time.sleep(1)
	
def kill_demon_realtime_RLS_R(): 
...
# 확인된 변수로 데몬을 실행 한다
def run_demon_realtime_RLS_R(arg): 
	...
	cmd = "node %s/realtime_RLS.js %s 2>&1 & " % (ITS_rls_r_path, arg)
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
...
def restart_its(): 
	cmd = "python /home/pi/optex_RLS_R/run_optex.pyc"
	print(cmd)
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
...
def create_table_w_log_RLS_R(postfix=''): # create_table_w_log(uniqueOfSensor)
	try:
...
def insert_event_RLS_R(tableName, w_evt_id=0, w_evt_X=0, w_evt_Y=0, w_evt_S=0, w_evt_zone=''): 
...
def read_table_w_cfg_sensorID(): ###################### Optex REDSCAN
	query = "SELECT * FROM " + ECOS_table_prefix + ECOS_table_RLS_R + " WHERE w_sensor_disable = 0" + " ORDER BY wr_id DESC" 
	try:
		conn = MySQLdb.connect(host=db_host, user=db_user, passwd=db_pass, db=db_name, charset='utf8', use_unicode=True) 
		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute(query)
...
def read_table_w_cfg_sensor_all(wr_id=''): # Optex Microwave
	query = "SELECT * FROM " + ECOS_table_prefix + ECOS_table_RLS_R + " WHERE wr_id = " + wr_id + " AND w_sensor_disable = 0 "
...
def read_field_w_cfg_status(wr_id=''): 
	query = "SELECT w_sensor_stop, w_sensor_reload, w_sensor_disable, w_alarm_disable FROM " + ECOS_table_prefix + ECOS_table_RLS_R + " WHERE wr_id = " + wr_id
	try:
		conn = MySQLdb.connect(host=db_host, user=db_user, passwd=db_pass, db=db_name, charset='utf8', use_unicode=True) 
		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute(query)
...
def set_reload_w_cfg_reload(wr_id=''): 
	query = "UPDATE " + ECOS_table_prefix + ECOS_table_RLS_R + " SET w_sensor_reload = '0' WHERE wr_id = " + wr_id
	try:
		conn = MySQLdb.connect(host=db_host, user=db_user, passwd=db_pass, db=db_name, charset='utf8', use_unicode=True) 
		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute(query)
