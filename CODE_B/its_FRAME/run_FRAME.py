...
# 모니터링을 위한 지도파일을 생성한다.
def make_FRAME_map():
	jquery = "{}/jquery/jquery-3.1.1.min.js".format(share["path"]["common"])
	__script_jquery_js__ = "<script>"+open(jquery, "r").read()+"</script>"

	# jquery_ui = "{}/jquery/jquery-ui-1.12.1/jquery-ui.js".format(share["path"]["common"])
	# __script_jquery_ui_js__ = "<script>"+open(jquery_ui, "r").read()+"</script>"
	# jquery_ui_css = "{}/jquery/jquery-ui-1.12.1/jquery-ui.css".format(share["path"]["common"])
	# __script_jquery_ui_css__ = "<script>"+open(jquery_ui_css, "r").read()+"</script>"

	# bootstrap_js = "{}/bootstrap/js/bootstrap.min.js".format(share["path"]["common"])
	# __style_bootstrap_js__ = "<script>"+open(bootstrap_js, "r").read()+"</script>"
	# bootstrap_css = "{}/bootstrap/css/bootstrap.min.css".format(share["path"]["common"])
	# __style_bootstrap_css__ = "<style>"+open(bootstrap_css, "r").read()+"</style>"
	
	# bootstrap_toggle_js = "{}/bootstrap/js/bootstrap-toggle.min.js".format(share["path"]["common"])
	# __style_bootstrap_toggle_js__ = "<script>"+open(bootstrap_toggle_js, "r").read()+"</script>"
	# bootstrap_toggle_css = "{}/bootstrap/css/bootstrap-toggle.min.css".format(share["path"]["common"])
...
def read_table_w_cfg_frame(): ##
	query = "SELECT * FROM " + share["table"]["frame"] + " WHERE w_frame_disable = 0" + " ORDER BY wr_id DESC" 
	try:
...
def kill_demon_FRAME(): 
	cmd = "kill $(ps aux | grep ' FRAME.js' | awk '{print $2}')"
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	time.sleep(1)
	return "kill_demon_FRAME"
	
def run_demon_FRAME(cfgJson): 
	cmd = "node FRAME.js {} 2>&1 & ".format(cfgJson)
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return "run_demon_FRAME"
...
def requestParse(content):
	...
	try:
		...
		# return "requests.get Error: {}".format(url)
		return 0
...
if __name__ == '__main__':
	...
	# owner = readConfig('/home/pi/FRAME/config.json') 
	# owner['status'] = share['status'].copy()
	# owner['mysql'] = share['mysql'].copy()

	## FRAME 정보 수집
	portIn = share["port"]["frame"]["portIn"] # 33000
	portOut = share["port"]["frame"]["portOut"] # 33100

	owner = {}
	owner['interface'] = {}
	owner['interface']['port_in'] = portIn
	owner['interface']['port_out'] = portOut

	# row = read_table_w_cfg_frame()
	...
	owner['file'] = {}
	owner['file']['html_source'] = share["path"]["frame"]+'/FRAME.html'
	owner['file']['html_target'] = share["path"]["frame"]+'/index_'+str(owner['interface']['port_in'])+'.html'
	
	# owner['camera'] = {}
	# owner['camera'] = readConfig('/home/pi/FRAME/camera.json')
	...
	# # owner['sensor']['direction'] = row['w_frame_direction']
	# # owner['sensor']['gpio_id_L'] = row['w_frame_detect_L']
	# # owner['sensor']['gpio_id_R'] = row['w_frame_detect_R']
	# # owner['sensor']['gpio_id_L'] = row['w_frame_detect_L']
	# # owner['sensor']['trigger_L'] = row['w_frame_trigger_L']
	# # owner['sensor']['trigger_R'] = row['w_frame_trigger_R']
	# # owner['sensor']['speed'] = {}
	...
	###############################################
	## 파일 config.json내용 저장
	cfgJson = share["path"]["frame"]+'/frame_'+str(owner['interface']['port_in'])+'.json'
	saveConfig(owner,cfgJson) ## 저장
	
	print(kill_demon_FRAME())
	make_FRAME_map()
	print(run_demon_FRAME(cfgJson))
