...
function getEvtLogOneDay(sensorID='', callback) { // 팬스별 하루치 로그 읽기 
	con.connect(function(err) {
		...
		con.query(sql, function (err, result, fields) {
			...
			callback(null, result); // 결과물(result)을 전송한다
			// console.log(result);
		});
	});
}
...
function ptzOnvifCtlGroup(client, senId) {
	...
	presetIs.forEach(function(camID) {
		if(ims.sensor[senId].preset[camID]) { // 위치 값이 있으면
...
function ipRelayWrite(addr, port, id, state, msg) {
	...
	console.log("ITS_ACU", cfg.interface.acu.ip, cfg.interface.acu.port, cfg.interface.acu.id, cfg.interface.acu.time, state, msg);
}
...
require('net').createServer(function (client) {
	...
	*/
	...
	client.on('data', function (data) { // data 변수에 포트로부터 입력된 값을 저장 하다.
		// id=g500t900_192_168_0_102_0001,name=Relay 01,beep=1,status=1,shot=,video=,count=1,block=0,msg=Active_Event
...
io.sockets.on('connection', function (socket) {
	...
	socket.on('ptzOnvifCtlGroup', function(data) { // 시스템 어카운트 정보 저장
		// 접속한 현재의 아이피가 어드민그룹에 포함되어 있으면 PTX 허용
		if (cfg.filterIP.admin.indexOf(clientIP) > -1) { // admin 사용자만 PTZ 기능 허용
			ptzOnvifCtlGroup(clientIP, data);
