...
def readConfig(filename='config.json'):
	with open('%s'%filename) as json_file:  
...
def bridgeInterface(virtualIP): ## 가상아이피 설정 # sudo ifconfig eth1 169.254.254.1 up
	cmd = "sudo ifconfig eth1 %s up" % (virtualIP)
...
def setSystemIP(newIP, newNM, newGW): # IP, NETMASK, GATEWAY 설정
	...
	if newIP and curIP and curIP != newIP:
		# 실시간 네트워크 환경 적용
		cmd = "sudo ifconfig %s %s" % (iface, newIP)
		os.system(cmd)

	if newNM and curNM and curNM != newNM:
		# 실시간 네트워크 환경 적용
		cmd = "sudo ifconfig %s netmask %s" % (iface, newNM)
		os.system(cmd)

	if newGW and curGW and curGW != newGW:
		# 실시간 네트워크 환경 적용
		cmd = "sudo route add default gw %s" % (newGW)
		os.system(cmd)

	print ('IP:{} NM:{} GW:{}'.format(newIP, newNM, newGW))

def masquerade(masq): # sensor.addr, interface.portMasq, 
...
def main ():
	try:
		...
		__var_data_config__ = json.dumps(cfg)

		__var_data_language__ = json.dumps(lan)
		...
		with open(cfg["path"]["spotter"]+cfg["file"]["html_src"], 'r') as templet_file:
			...
			tmp_its_tmp = tmp_its_tmp.replace('__var_data_config__', __var_data_config__)
			tmp_its_tmp = tmp_its_tmp.replace('__var_data_language__', __var_data_language__)
...
if __name__ == '__main__':
	...
	lan = readConfig("language.json") ## 환경변수 읽기
	...
	## ITS System IP 설정
	setSystemIP(cfg["interface"]["its"]["address"], cfg["interface"]["its"]["netmask"], cfg["interface"]["its"]["gateway"])
	...
	# 센서와의 통신을 위한 보조 인터페이스(eth1) 연결
	subIFace = bridgeInterface(cfg["interface"]["portAddr"])
	if check_ping(cfg["interface"]["portAddr"]): ## 센서 
		logger.info("Pass Bridge Interface Check(%s)."%cfg["interface"]["portAddr"])
		print ("\n\tPass Bridge Interface Check")
		cfg["actionLog"].append( "%s Pass Bridge Interface Check."%str(time.time()) )
	else:
		logger.warning("Error Bridge Interface Check(%s)."%cfg["interface"]["portAddr"])
		print ("\n\tError Bridge Interface Check")
		cfg["actionLog"].append( "%s Error Bridge Interface Check."%str(time.time()) )
		exit()
	...
	if check_ping(cfg["sensor"]["addr"]): ## 센서 테스트 결과 값
		...
		print ("\n\tError Sensor Connection Check or Network Interface!!")
	...
	cfg["device"] = {}
	cfg["device"]["addr"] = cfg["sensor"]["addr"] # "192.168.168.30"
	cfg["device"]["port"] = cfg["interface"]["portMasq"] # 10001 # 52101 
	cfg["device"]["masq"] = True # w_device_masq
	masquerade(cfg["device"]) # sensor.addr, interface.portMasq, 
