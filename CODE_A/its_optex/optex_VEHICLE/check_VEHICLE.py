...
def kill_demon_realtime_RLS(): 
	cmd = "pkill -9 -ef check_RLS.js 2>&1" 
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	time.sleep(1)
	return p
	
def run_demon_check_RLS(arg): 
	cmd = "cd /home/pi/optex_VEHICLE; node check_RLS.js %s 2>&1 & " % (arg)
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p

# 센서 아이피 확인 
def check_sensor(): # 50001 포트가 살아있는지 확인 한다.
    # return os.system("ping -c1 -W1 " + sensorIP + " > /dev/null") # IF RETURN 0 THAT Network Active ELSE Network Error
	sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	sock.settimeout(0.1)
	result = sock.connect_ex((cfg["sensor"]["sensorIP"], cfg["sensor"]["sensorPort"]))
	sock.close()
	return result

def create_table_RLS_RAW(tableName=''): # 테이블 생성 ITS_VEHICLE_row_table = "w_log_RLS_RAW"
	try:
...
def insert_event_RLS_RAW(tableName, w_evt_id=0, w_evt_X=0, w_evt_Y=0, w_evt_S=0, w_evt_zone=''): 
	query = "INSERT INTO "+cfg["table"]["vechile"]+"_"+tableName+"(w_evt_id, w_evt_X, w_evt_Y, w_evt_S, w_evt_zone) VALUES(%s, %s, %s, %s, %s)"
...
def main ():
	...
	sockS30 = socket.create_connection((cfg["sensor"]["sensorIP"], cfg["sensor"]["sensorPort"])) # 센서 연결
	...
	print('Running RLS Realtime Monitoring - http://%s:%s' %(cfg["interface"]["IP"],cfg["interface"]["portOut"]))
	...
	target = "%s/realtime_RLS_%s.html" % (path, cfg["interface"]["portIn"]) # Optex Theme
...
'''
	...
	# 템플릿 파일에 확인된 정보 적용
	replacements = {tag_model:result_cmd12, tag_version:result_cmd02, tag_zone:html_zone, tag_area:result_cmd04, tag_mask:result_cmd11_M, tag_allocate:result_cmd11_A, tag_grid:html_grid, tag_frame:html_frame, tag_over:html_over, tag_viewBox:html_viewBox, tag_style:html_style, inc_jq:__script_jquery_js__, inc_jq_ui:__script_jquery_ui_js__, inc_jq_ui_css:__style_jquery_ui_css__, inc_svg_pan_zoom:__svg_pan_zoom__}
	...
	run_demon_check_RLS(ECOS_unionTable)
	...
	# 데이터베이스 테이블 생성
	tableName = cfg["sensor"]["sensorIP"].replace(".", "_")
			...
			if result_cmd06:  # 42949672541081,422,676,355|42949672541081,259,861,54|
				...
				for singleEvent in data:
					if singleEvent: # 42949672541081,422,676,355
						...
						# print sensorIP, singleEvent, eventZone
						insert_event_RLS_RAW(tableName, eleEvt[0], eleEvt[1], eleEvt[2], eleEvt[3], eventZone)
						realtime_monitoring(cfg["interface"]["IP"],cfg["interface"]["portIn"],singleEvent)
	finally:
...
if __name__ == '__main__':
	...
	com_cfg = '/home/pi/optex_VEHICLE/config.json'
	cfg = readConfig(com_cfg)

	varPort = int(cfg["sensor"]["sensorIP"].split('.')[2]) + int(cfg["sensor"]["sensorIP"].split('.')[3])
	tmpPortIn = cfg["sensor"]["sensorPort"] + varPort
	tmpPortOut = tmpPortIn + 1000

	## JS Code를 위한 JSON 형식의 환경파일 생성
	own_cfg = '/home/pi/optex_VEHICLE/config_'+str(tmpPortIn)+'.json'

	if os.path.isfile(own_cfg):
		cfg = readConfig(own_cfg)
	else:
		cfg = readConfig(com_cfg)
		cfg["masking"] = {}
		...
		cfg["interface"] = {}
		cfg["interface"]["IP"] = get_ip_address('eth0')
		cfg["interface"]["portIn"] = tmpPortIn # sensorIP = '192.168.168.30' -> 50199 = 50001 + 198
		cfg["interface"]["portOut"] = tmpPortOut # sensorIP = '192.168.168.30' -> 51199 = 1000 + 50001 + 198

	saveConfig(cfg,own_cfg)

	if check_sensor(): # 50001 포트가 살아있는지 확인 한다.
		exit("*** Sensor Test ERROR ***\n\tPlease check Sensor's IP(%s)" % cfg["sensor"]["sensorIP"])
							
	ECOS_unionTable = '%s %s' % (cfg["interface"]["portIn"], cfg["interface"]["portOut"])
	print ECOS_unionTable
