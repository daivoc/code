...
table_BSS = c.ECOS_table_prefix+c.ECOS_table_SPEED

def get_sensor_serial_BSS(): # Optex Microwave
	query = "SELECT wr_id, wr_subject, w_sensor_serial, w_email_Addr FROM " + table_SPEED + " WHERE w_sensor_disable = 0 AND w_email_Addr != ''" 
	try:
...
def export_report_by_DUE(dueDate='1 DAY'): # or '1 WEEK'
	w_cfg_sensor_list_BSS = get_sensor_serial_BSS()
	for row in w_cfg_sensor_list_BSS:
		ECOS_myID = row["wr_id"]
		ECOS_serialNo = row["w_sensor_serial"]
		ECOS_dbName = c.ITS_sensor_log_table + "_" + ECOS_serialNo
