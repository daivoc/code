...
def kill_demon_realtime_RLS(): 
	cmd = "pkill -9 -ef realtime_RLS 2>&1" 
...
def run_demon_realtime_RLS(arg): 
	cmd = "cd /home/pi/optex_BIND; node check_RLS.js %s 2>&1 & " % (arg)
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p
...
def create_table_RLS_RAW(tableName=''): # 테이블 생성 ITS_BIND_row_table = "w_log_RLS_RAW"
	try:
...
def insert_event_RLS_RAW(tableName, w_evt_id=0, w_evt_X=0, w_evt_Y=0, w_evt_S=0, w_evt_zone=''): 
	query = "INSERT INTO "+cfg["table"]["bind"]+"_"+tableName+"(w_evt_id, w_evt_X, w_evt_Y, w_evt_S, w_evt_zone) VALUES(%s, %s, %s, %s, %s)"
...
if __name__ == '__main__':
	sensor_IP = '192.168.168.30'
	if check_sensor(sensor_IP): # 50001 포트가 살아있는지 확인 한다.
	...
	com_cfg = '/home/pi/optex_BIND/config.json'
	own_cfg = '/home/pi/optex_BIND/config_'+str(nodeIn)+'.json'
	...
	if os.path.isfile(own_cfg):
		...
		cfg["interface"] = {}
		cfg["interface"]["portIn"] = nodeIn
		cfg["interface"]["portOut"] = nodeOut
		cfg["sensor"] = {}
