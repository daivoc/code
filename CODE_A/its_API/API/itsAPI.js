...
var cfg = JSON.parse(fs.readFileSync('/home/pi/API/itsAPI.json', 'utf8')); // 환경 파일 읽기
var cfgCam = JSON.parse(fs.readFileSync('/home/pi/API/camera.json', 'utf8')); // 환경 파일 읽기
...
function socket_write(host, port, data) {
	...
	data.forEach(function(value) {
		...
		push_gLog('<font color=gray>Sent to '+host+':'+port+'</font> '+JSON.stringify(value)); // Log -> index.html, 로그전송
...
function btn_status_gpio(id, status, source) { // 예: id:io01/pw01/.., status:0/1, source:source
	//////////////////////////////////////////////////////////
	// 1. 외부 명령은 itsAPI.pyc로 전송한다.
	// 2. 버튼 상태를 index.html로 전송한다.
	//////////////////////////////////////////////////////////

	// 센서 이벤트에 외부명령이 존재하면 
	// 내용을 socket_write를 통해 itsAPI.pyc로 전송한다.
	if(source == 'watch' && cfg['command'][id] && cfg['execution'][id]) { // (센서이벤트발생 && 명령어가 있고 && 실행이 활성)
		if((status == cfg['trigger'][id])) { // 트리거와 상태값이 같으면 
			try {
				...
				socket_write(obj['host'], obj['port'], obj['data']); // 명령은 itsAPI.pyc로 전송
...
function btn_enable(myKey, myID, status) {
	if(myID == 'admin') {
		var btn_group = "<button id='save' type='button' class='btn btn-light ctrlBtn'>Save</button><button id='restart' type='button' class='btn btn-info ctrlBtn'>Restart</button><button id='reboot' type='button' class='btn btn-info ctrlBtn'>Reboot</button><div id='gLogBtn' class='gLogBtn'>Log</div><div id='cfgSet' class='cfgSet'>Config</div><div id='setup' class='setup'>Setup</div>";
	} else if(myID == 'manager') {
		var btn_group = "<button id='save' type='button' class='btn btn-light ctrlBtn'>Save</button><button id='restart' type='button' class='btn btn-info ctrlBtn'>Restart</button><button id='reboot' type='button' class='btn btn-info ctrlBtn'>Reboot</button><div id='gLogBtn' class='gLogBtn'>Log</div><div id='cfgSet' class='cfgSet'>Config</div>";
	} else {
		var btn_group = "";
	}
	try {
...
// function btn_status_audio(data) {
// 	io.sockets.emit('btn_status_audio', data);
// }

// function btn_status_set_time(data) {
// 	io.sockets.emit('btn_status_set_time', data);
// }
...
io.sockets.on('connection', function (socket) {
	...
	socket.on('btnStopAudioClick', function(data) { // 센서 이벤트 강제 발생
		if(data['password'] == cfg['permission']['password'][data['myID']]) {
			...
			try {
				socket_write(cfg.tcpIpPort.staticAddress, cfg.portAPI, data['command']); // 명령은 itsAPI.pyc로 전송
				// console.log(cfg.tcpIpPort.staticAddress, cfg.portAPI, data['command']);
				push_gLog('<font color=green>btnStopAudioClick</font> '+JSON.stringify(data['command'])); // 데이터가 오브잭트이어서 JSON.stringify해줌 Log -> index.html, 로그전송
			} catch (e) {
	...
	socket.on('btnSetTimeClick', function(data) { // 센서 이벤트 강제 발생
		if(data['password'] == cfg['permission']['password'][data['myID']]) {
			...
			try {
				socket_write(cfg.tcpIpPort.staticAddress, cfg.portAPI, data['command']); // 명령은 itsAPI.pyc로 전송
				// console.log(cfg.tcpIpPort.staticAddress, cfg.portAPI, data['command']);
				push_gLog('<font color=green>btnSetTimeClick</font> '+JSON.stringify(data['command'])); // 데이터가 오브잭트이어서 JSON.stringify해줌 Log -> index.html, 로그전송
			} catch (e) {
	...
	socket.on('restart_self', function(data) {
		exec_command('python ./run_itsAPI.pyc');
	});

	socket.on('save_self', function (data) { // itsAPI.html로 부터 변경된 값을 config.json에 저장 한다.
		// console.log(data['config']);
		var own = JSON.parse(fs.readFileSync('/home/pi/API/config.json', 'utf8')); // 환경 파일 읽기
		...
		fs.writeFileSync('/home/pi/API/config.json', JSON.stringify(own, null, 4), 'utf8'); // 환경 파일 저장
		// var dateTime = new Date();
		// fs.writeFileSync('/home/pi/API/'+dateTime+'.json', JSON.stringify(own, null, 4), 'utf8'); // 환경 파일 저장
		...
		// socket_write(cfg.tcpIpPort.staticAddress, cfg.portAPI, [{'global_var':cfg}]); // 전역변수 값을 itsAPI.pyc로 전송
		socket_write(cfg.tcpIpPort.staticAddress, cfg.portAPI, [{'global_var':own}]); // 전역변수 값을 itsAPI.pyc로 전송
	});
...
require('net').createServer(function (socket) {
	//////////////////////////////////////////////////////////
	// 수신용 코드
	// 2. 외부(itsAPI.pyc) -> portIn(18040) -> itsAPI.js -> itsAPI.html -> portOut(18080)
	...
	socket.on('data', function (data) { // 외부 portIn 으로 부터 받은 데이터(예:'gpioID=io01,source=message')
		...
		obj = JSON.parse(data)
		if(obj.name == 'gpioID') {
			// itsAPI.pyc로 부터 변화된 릴레이 상태를 적용 한다.
			...
			push_gLog('<font color=orange>From Server(pyc)</font> '+obj.value); // Log -> index.html, 로그전송
			...
			// itsAPI.pyc(systemAction) -> itsAPI.js(here)
			...
			// itsAPI.pyc(systemAction) -> itsAPI.js(here) -> itsAPI.pyc(btn_status_gpio)
