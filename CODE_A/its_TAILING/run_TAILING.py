...
def kill_demon_TAILING():
	# 아래 grep에서 스페이스를 포함해야 자신(run_TAILING.pyc)을 죽이지 않는다
	# cmd = "pkill -9 -ef $(ps aux | grep '/tailing_' | awk '{print $2}')"
	cmd = "pkill -9 -ef TAILING/TAILING 2>&1" ## 주의) 직전에 실행된 프로세서를 죽이는 오류 발생 가능
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return "kill_demon_TAILING"

def run_demon_TAILING_PY(cfgJson): 
	cmd = "python %s/TAILING.pyc %s 2>&1 & " % (share['path']['tailing'],cfgJson)
	# print cmd
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p
...
def read_table_w_cfg_tailing(): ## 사용자 환경 변수 로딩
	query = "SELECT * FROM " + share['table']['tailing'] + " WHERE w_sensor_disable = 0" + " ORDER BY wr_id DESC" 
	try:
...
def main():
	tableRow = read_table_w_cfg_tailing()
	...
	for row in tableRow:
		...
		owner['user_table'] = {}
		owner['user_table']['board']    = share['table']['tailing']
		owner['user_table']['wr_id']    = row['wr_id']

		owner['interface'] = {}
		owner['interface']['port_JS_in']   = share['port']['tailing']['portIn'] + row['wr_id']
		owner['interface']['port_JS_out']  = share['port']['tailing']['portOut'] + row['wr_id']
		owner['interface']['port_PY_in']   = share['port']['tailing']['portIn'] + row['wr_id'] + 1
		owner['interface']['port_PY_out']  = share['port']['tailing']['portOut'] + row['wr_id'] + 1

		if row['w_device_id']: # Main Board or USB 카메라가 존재하면
			owner['interface']['device_id'] = int(row['w_device_id'])
		else:
			owner['interface']['device_id'] = 0

		owner['interface']['w_sensor_url'] = row['w_sensor_url']

		owner['file'] = {}
		owner['file']['conf_common']    = '/home/pi/common/config.json'
		owner['file']['html_source']    = share['path']['tailing']+'/TAILING.html'
		owner['file']['html_target']    = share['path']['tailing']+'/index_'+str(owner['interface']['port_JS_in'])+'.html'
		owner['file']['conf_tailing']    = share['path']['tailing']+'/tailing_'+str(owner['interface']['port_JS_in'])+'.json'

		owner['file']['log_tailing']     = share['path']['log']+'/'+row['w_sensor_serial']+'.log'

		owner['file']['image_folder']   = share['path']['img']+'/tailing_'+row['w_sensor_serial']
		# owner['file']['image_NIR']      = share['path']['img']+'/tailing_'+row['w_sensor_serial']+'/NIR'
		owner['file']['image_tailing']  = share['path']['img']+'/tailing_'+row['w_sensor_serial']+'/tailing'
		owner['file']['image_manual']   = share['path']['img']+'/tailing_'+row['w_sensor_serial']+'/manual'
		owner['file']['image_unknown']  = share['path']['img']+'/tailing_'+row['w_sensor_serial']+'/unknown'
		owner['file']['image_final']    = share['path']['img']+'/tailing_'+row['w_sensor_serial']+'/final'
		...
		owner['sensor']['size_x']       = 640
		owner['sensor']['size_y']       = 480
		...
		owner['opencv']['tail_w']       = row['w_opencv_tail_w'] # 400 # row['w_opencv_tail_w']
		owner['opencv']['tail_h']       = row['w_opencv_tail_h'] # 300 # row['w_opencv_tail_h']
		owner['opencv']['tail_x']       = row['w_opencv_tail_x'] # 0 # row['w_opencv_tail_x']
		owner['opencv']['tail_y']       = row['w_opencv_tail_y'] # 0 # row['w_opencv_tail_y']
		owner['opencv']['tail_over_x']  = row['w_opencv_tail_over_x'] # 100 # Oversize - 다수로 의심
		owner['opencv']['tail_over_y']  = row['w_opencv_tail_over_y'] # 100 # Oversize - 다수로 의심
		owner['opencv']['tail_pixel']   = row['w_opencv_tail_pixel'] # 500 # 픽셀 임계치
		...
		owner['log_table']['tbl_live']  = 'w_log_tailing_live_'+row['w_sensor_serial']
		owner['log_table']['tbl_min']   = 'w_log_tailing_min_'+row['w_sensor_serial']
		owner['log_table']['tbl_hour']  = 'w_log_tailing_hour_'+row['w_sensor_serial']
		owner['log_table']['tbl_day']   = 'w_log_tailing_day_'+row['w_sensor_serial']
		owner['log_table']['tbl_week']  = 'w_log_tailing_week_'+row['w_sensor_serial']
		owner['log_table']['tbl_month'] = 'w_log_tailing_month_'+row['w_sensor_serial']
		owner['log_table']['tbl_sum']   = 'w_log_tailing_sum_'+row['w_sensor_serial']
		owner['log_table']['tbl_life']  = 30 # days 이후 삭제

		owner['log_pmt'] = {}
		owner['log_pmt']['tbl_live']  = 'w_log_permit_live_'+row['w_sensor_serial']
		owner['log_pmt']['tbl_min']   = 'w_log_permit_min_'+row['w_sensor_serial']
		owner['log_pmt']['tbl_hour']  = 'w_log_permit_hour_'+row['w_sensor_serial']
		owner['log_pmt']['tbl_day']   = 'w_log_permit_day_'+row['w_sensor_serial']
		owner['log_pmt']['tbl_week']  = 'w_log_permit_week_'+row['w_sensor_serial']
		owner['log_pmt']['tbl_month'] = 'w_log_permit_month_'+row['w_sensor_serial']
		...
		# 메인 프로그램 실행
		run_demon_TAILING_PY(owner['file']['conf_tailing'])
