...
/* 
- 설정시 목록보기를 기본 15에서 등록가능한 최대 수로 변경
- 오더값은 목록의 출력 순서를 정한다.
- 오더값의 기본은 write_update에서 9999로 재설정 한다.
- w_sub_title, w_order
*/
...
// w_group_main
function list_w_group_main() {
    global $g5, $bo_table;
	$write_table = $g5['write_prefix'] . $bo_table;
	$sql = " SELECT w_group_main FROM $write_table ";
	$manual_group = array();
	...
	$list_w_group_id = '<datalist id="list_group_main">';
	foreach ($manual_group as $key => $value) {
		$list_w_group_id .='<option value='.$key.'>';
	}
	$list_w_group_id .= '</datalist>';
	return $list_w_group_id;
}
// w_group_sub
function list_w_group_sub() {
    global $g5, $bo_table;
	$write_table = $g5['write_prefix'] . $bo_table;
	// if($write['w_group_main']){
		// $whereIS = " WHERE w_group_main = '".$write['w_group_main']."'";
	// } else {
		// $whereIS = "";
	// }
	// $sql = " SELECT w_group_sub FROM $write_table ".$whereIS;
	$sql = " SELECT w_group_main, w_group_sub FROM $write_table ";
	$manual_group = array();
	$result = sql_query($sql);
	while ($row = sql_fetch_array($result)) {
		if($row[w_group_sub]){
...
// PHP 배열을 자바스크립트 배열로 변환 및 적용한다.
$js_array = json_encode(list_w_group_sub());
echo "<script>var group_sub = ". $js_array . ";\n</script>";
?>
<script>
$(document).ready(function(){
	$("#w_group_main").change(function() {
		$("#list_group_sub").empty();
		for (var key in group_sub[$(this).val()]) {
			if (group_sub[$(this).val()].hasOwnProperty(key)) {           
				// console.log(key, dictionary[key]);
				$("#list_group_sub").append("<option value='"+key+"'>");
			}
		}
	});
});
...
<section id="bo_w" class="container">
	...
	<div class="tbl_wrap table-responsive">
		...
		<tr>
			...
			<td>
				<select name="ca_name" id="ca_name" required class="required form-control input50P" >
		...
		<tr>
			<th scope="row"><label for="ca_name">분류<strong class="sound_only">필수</strong></label></th>
			<td>
				<input list="list_group_main" type="text" name="w_group_main" value="<?php echo $write['w_group_main'] ?>" id="w_group_main" required class="form-control input25P required" size="50" placeholder="키워드(주)">
				<?php echo list_w_group_main(); ?>
				<input list="list_group_sub" type="text" name="w_group_sub" value="<?php echo $write['w_group_sub'] ?>" id="w_group_sub" class="form-control input50P" size="50" placeholder="키워드(부)">
				<datalist id="list_group_sub"></datalist>
				<?php // echo list_w_group_sub(); ?>
				<input type="text" name="w_order" value="<?php echo $write['w_order'] ?>" id="w_order" class="form-control input25P" size="50" placeholder="출력 순서">
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<div id="autosave_wrapper">
					...
					<textarea type="text" name="w_sub_title" id="w_sub_title" class="form-control" size="50" maxlength="255"><?php echo $write['w_sub_title'] ?></textarea>
