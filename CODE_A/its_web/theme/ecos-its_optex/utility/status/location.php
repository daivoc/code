...
// 목록보기 링크
$list_view_link = 'list.php';

// USB 드라이버 관련 device port
global $g5, $bo_table;

// TCP 센서 좌표값 추출
$bo_table_tcp_info = G5_CU_CONF_TCP;
// 목록보기 링크
$write_table = $g5['write_prefix'] . $bo_table_tcp_info;
...
while ($row = sql_fetch_array($result)) {
	// ['동문 북능선 3번 센서', 37.5359859, 127.0885968, URL],
	$location_info .= "['".$row['wr_subject']."', ".$row['w_sensor_lat_s'].", ".$row['w_sensor_lng_s'].", ".$row['w_sensor_lat_e'].", ".$row['w_sensor_lng_e'].",'./".$list_view_link."?commType=ttyUSB&devideID=".$row['w_device_id']."&sensorID=".$row['w_sensor_id']."&subject=".$row['wr_subject']."'],";
	// $location_path .= "{lat: ".$row['w_sensor_lat_s'].", lng: ".$row['w_sensor_lng_s']."},";
	$w_sensor_lat += $row['w_sensor_lat_s']; 
	$w_sensor_lat += $row['w_sensor_lat_e'];
	$w_sensor_lng += $row['w_sensor_lng_s'];
...
$sql = " SELECT * FROM $write_table WHERE wr_10 = 'Enable' ORDER BY w_device_id ASC";
$result = sql_query($sql);
$num_rows += sql_num_rows($result); // RXTX 센서 갯수에 릴레이 자료 갯수 추가
while ($row = sql_fetch_array($result)) {
	// ['동문 북능선 3번 센서', 37.5359859, 127.0885968, URL],
	$location_info .= "['".$row['wr_subject']."', ".$row['w_sensor_lat_s'].", ".$row['w_sensor_lng_s'].", ".$row['w_sensor_lat_e'].", ".$row['w_sensor_lng_e'].",'./".$list_view_link."?commType=GPIO&devideID=".$row['w_device_id']."&sensorID=".$row['w_sensor_id']."&subject=".$row['wr_subject']."'],";
	// $location_path .= "{lat: ".$row['w_sensor_lat_s'].", lng: ".$row['w_sensor_lng_s']."},";
	$w_sensor_lat += $row['w_sensor_lat_s']; 
	$w_sensor_lat += $row['w_sensor_lat_e'];
	$w_sensor_lng += $row['w_sensor_lng_s'];
