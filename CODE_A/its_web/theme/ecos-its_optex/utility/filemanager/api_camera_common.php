...
$default_timezone = 'Asia/Seoul'; // UTC+3

// Root path for file manager
// $root_path = $_SERVER['DOCUMENT_ROOT'];
$root_path = '/var/www/html/its_web/theme/ecos-its_optex/user/image/camera';

// Root url for links in file manager.Relative to $http_host. Variants: '', 'path/to/subfolder'
// Will not working if $root_path will be outside of server document root
// $root_url = '';
$root_url = 'theme/ecos-its_optex/user/image/camera';
...
if (isset($_GET['view'])) {
	...
	<div class="path">
		...
		<p>
			...
			<b><a href="<?php echo fm_enc($file_url) ?>" target="_blank">🔗</i> Open</a></b> &nbsp;
			<?php
			// ZIP actions
			if ($is_zip && $filenames !== false) {
...
<th class="hide" style="width:12%">Modified</th>
<?php if (!FM_IS_WIN): ?><th class="hide" style="width:6%">Perms</th><th class="hide" style="width:10%">Owner</th><?php endif; ?>
...
<td>Folder</td><td class="hide"><?php echo $modif ?></td>
<?php if (!FM_IS_WIN): ?>
<td class="hide"><a title="Change Permissions" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a></td>
<td class="hide"><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
<?php endif; ?>
<td>
<a title="Delete" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Delete folder?');">❌</a>
<a title="Rename" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;">✏️</a>
<a title="Copy to..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>">📋</a>
<a title="Direct link" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank">🔗</a>
</td></tr>
	<?php
	flush();
}
...
<td class="hide"><?php echo $modif ?></td>
<?php if (!FM_IS_WIN): ?>
<td class="hide"><a title="Change Permissions" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a></td>
<td class="hide"><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
<?php endif; ?>
<td>
<a title="Delete" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Delete file?');">❌</a>
<a title="Rename" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;">✏️</a>
<a title="Copy to..." href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>">📋</a>
<a title="Direct link" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank">🔗</a>
<a title="Download" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>">📥</a>
</td></tr>
	<?php
	flush();
}
...
<p class="hide path"><a href="#" onclick="select_all();return false;"><i class="icon-checkbox"></i> Select all</a> &nbsp;
...
<a title="Upload files" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload">📤</a>
<a title="New folder" href="#" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;">📁</a>
<?php if (FM_USE_AUTH): ?><a title="Logout" href="?logout=1">💨</a><?php endif; ?>
</div>
		<?php
		$path = fm_clean_path($path);
...
#wrapper{max-width:600px;min-width:400px;margin:10px}
.path{padding:4px 7px;border:1px solid #ddd;background-color:#fff;margin-bottom:10px;font-size: large;}
...
{
	?>
<p class="center"><small>ECOS File Manager</small></p>
</div>
<script>
