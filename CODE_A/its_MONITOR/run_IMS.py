...
def tbl_sensor(sns):
	result = ''
	for key in sorted(sns): # key는 센서 시리얼
		# result += "<table> <tr> <td colspan=4>%s</td> </tr> <tr> <td colspan=4>BBB</td> </tr> <tr> <td>A</td> <td>B</td> <td>C</td> <td>D</td> </tr> </table>" % sns[key]['subj']
		result += "<div class='group' id='tbl_"+key+"'> <div class='title'>%s</div> <div class='model'>%s</div> <div class='times'>-</div> <div class='event'> <span class='a'>A</span> <span class='b'>H</span> <span class='c'>E</span> <span class='d'>e</span> </div> <div class='level'></div> </div>" % (sns[key]['subj'], sns[key]['model'])
	return result
	
def tbl_camera(cam):
	result = ''
	for key in sorted(cam): # key는 카메라 시리얼
...
'''	
	for key in sorted(its): # key는 아이티에스 IP Addr임, addr로 정열함
...
if __name__ == '__main__':
	...
	ims['file'] = cfg['file'].copy() # fitScreen
	ims['timer'] = cfg['timer'].copy() # 
	...
	ims['program'] = cfg['program'].copy() #
	ims['userApi'] = cfg['userApi'].copy() # 
