...
def make_its_M_map():
	...
	## Box and Zone
	__tbl_zone__ = tbl_monitor(ims['zone']).encode('utf-8')
	# print __tbl_zone__

	__tbl_box__ = tbl_monitor(ims['box']).encode('utf-8')
	# print __tbl_box__
	
	## 카메라 테이블  
	__tbl_camera__ = tbl_monitor(ims['camera']).encode('utf-8')
	# print __tbl_camera__
	...
	__svg_content__ = '<svg x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;"></svg>'
	...
	with open(cfg["file"]["html_source"], 'r') as templet_file:
		...
		tmp_its_tmp = tmp_its_tmp.replace('__tbl_zone__', __tbl_zone__)
		tmp_its_tmp = tmp_its_tmp.replace('__tbl_box__', __tbl_box__)
...
## 확인된 변수로 데몬을 실행 한다
def run_its_M_map(): 
	cmd = "node %s/its_M_map.js 2>&1 & " % (cfg["path"]["mon_tbl"])
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p

## 확인된 변수로 데몬을 실행 한다
def run_procOnvif(camID): 
	cmd = "node %s/procOnvif.js %s 2>&1 & " % (cfg["path"]["mon_tbl"], camID)
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p
	
def run_ipCamView(): 
	cmd = "node %s/ipCamView.js 2>&1 & " % (cfg["path"]["mon_tbl"])
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p
	
## 확인된 변수로 데몬을 실행 한다
def run_ipCamInfo(): 
	cmd = "node %s/ipCamInfo.js 2>&1 & " % (cfg["path"]["mon_tbl"])
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p
...
def itsMemberConfig(field): ## 사용자 its의 특정필드 내용 확인
	query = "SELECT " + field + " FROM g5_member WHERE mb_id = 'its'" 
	try:
		conn = MySQLdb.connect(host=cfg["mysql"]["host"], user=cfg["mysql"]["user"], passwd=cfg["mysql"]["pass"], db=cfg["mysql"]["name"], charset='utf8', use_unicode=True) 
		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute(query)
...
def tbl_monitor(obj):
	result = ''
	for key in obj:
		result += "<div class='group' id='tbl_"+key+"'> <div class='title'>%s</div> <div class='model'>%s</div> <div class='times'>-</div> <div class='event'> <span class='a'>A</span> <span class='b'>B</span> <span class='c'>C</span> <span class='d'>D</span> </div> </div>" % (obj[key]['subj'], obj[key]['mapID'])
	return result
...
if __name__ == '__main__':
	...
	cfg = readConfig('./config.json') # '/home/pi/MONITOR/%s.json'
	icc = readConfig('./camera.json')
	lan = readConfig('./language.json')
	...
	audioName = itsMemberConfig('mb_2')
	audioTime = itsMemberConfig('mb_3')
	...
	relayAddr = itsMemberConfig('mb_5')
	relayPort = itsMemberConfig('mb_6')
	relayNumber = itsMemberConfig('mb_7')
	...
	###############################################
	## 파일 config.json내용 저장
	saveConfig(ims,cfg["path"]["mon_tbl"]+'/cfgIms.json') ## 저장
	...
	# if(cfg["runProgram"]["procOnvif"]): # 카메라 수만큼의 온비프 인터페이스 프로그램을 실행
		# for camID in ims['camera'].keys():
			# if icc[ims['camera'][camID]['model']]['isPTZ']: # 카메라 목록에서 PTZ 기능인 확인
				# print "ID:%s IP:%s Model:%s"%(camID, ims['camera'][camID]['addr'], ims['camera'][camID]['model'])
				# run_procOnvif(camID)
