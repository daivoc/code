...
// S1(에스원)
//////////////////////
function postCommand_S1(url, data) {
	request.post({
		headers: {"content-type" : "application/x-www-form-urlencoded"},
		// url:     "http://192.168.0.50:8022/SVMS/Event/optexfd",
		// body:    "dict_data=<EventMessage><Type>1700</Type><AlarmGroup>9</AlarmGroup><ID>1</ID><AlarmTime></AlarmTime></EventMessage>"
		url:     url,
		body:    "dict_data="+data
	}, function(error, response, body){
		// console.log(error);
...
function createTable() {
	con.connect(function(err) {
		...
		con.query(sql, function (err, result, fields) {
			...
			// console.log(result);
			var sql = "CREATE TABLE IF NOT EXISTS w_log_sensor_M ( w_id int(11) NOT NULL AUTO_INCREMENT, w_sensorId varchar(64) NULL DEFAULT '', w_shot varchar(255) NULL DEFAULT '', w_action varchar(32) NULL DEFAULT '', w_description varchar(64) NULL DEFAULT '', w_stamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (w_id) ) ENGINE=MyISAM  DEFAULT CHARSET=utf8"; 
...
function saveConfigJson() {
	fs.writeFile(cfg.path.webRoot+'/'+cfg.path.config+'/config.json', JSON.stringify(cfg, null, 4), (err) => {
		if (err) throw err;
...
function parseObj(obj) {
	for(var i = 0; i < obj.result.length; i++) {
		...
		if(cfg.maskSelect == 0 || (cfg.maskSelect == 1 && maskIs == 1) || (cfg.maskSelect == 2 && maskIs == 0)) {
			// 카메라 추적은 마스킹 조건과 알람조건 모두를 만족할때 실행
			// asix 카메라 PTZ Control
			...
			// http://192.168.0.50/get_post_receiver.php
			var data = "<EventMessage><Type>1700</Type><AlarmGroup>9</AlarmGroup><ID>1</ID><AlarmTime></AlarmTime></EventMessage>";
			for (item in cfg.userEnv.userSrvRequest) { // 사용자 Http Request는 기본적으로 4개 까지(config.json) 허용 한다.
				if(cfg.userEnv.userSrvRequest[item]) {
					var url = cfg.userEnv.userSrvRequest[item];
					postCommand_S1(url, data); // 에스원 이벤트 전송 테스트
					// console.log("maskID="+maskID+"&speed="+speed);
				}
			}
			//////////////////////////////////////
