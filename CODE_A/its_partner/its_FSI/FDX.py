...
def  replaceRule(zoneID, zoneName, eventKey, data):
	...
	__zone__ = cfg["device"]["zoneName"][zoneName] # __zone__ 이벤트 형태 : "Fault", "Intrusion", "OK", "Other", "Temper"
 	__value__ = cfg["device"]["value"][eventKey] # __value__ 이벤트 값(정수) : "9", "1", "2", "0", "8"
	__type__ = cfg["device"]["type"][eventKey] # __type__ 이벤트 형태 : "Fault", "Intrusion", "OK", "Other", "Temper"
...
def doAction(zoneID, dataJson):
	...
	if dataJson["command"] == "CommandMessage":
		...
		if dataJson["detectionEvent"] == "Intrusion":
			status = 1
		elif dataJson["detectionEvent"] == "Temper":
			status = 8
