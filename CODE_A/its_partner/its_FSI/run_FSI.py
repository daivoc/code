...
def main():
	...
	cfg["user"] = readConfig("/home/pi/FSI/config.json")
	...
	cfg["device"]["value"] = { # common -> config.json -> status
		...
		"Temper":int(elements[2]) if elements[2] else 8,
	...
	cfg["device"]["type"] = {
		...
		"Temper":elements[2] if elements[2] else "Temper",
	...
	cfg["local"]["port"] = share["port"]["fsi"] # 52001 
	cfg["local"]["zone"] = {}
