...
// GPIO 디바이스포트, 템퍼포트는 디바이스포트의 짝수로 한다.
function select_w_GPIO_id($cur_w_device_id, $arr_R) {
    global $g5, $bo_table;
	$write_table = $g5['write_prefix'] . $bo_table;
	$sql = " SELECT * FROM $write_table ";
    $result = sql_query($sql);
	...
	$select_w_device_id = '<select name="w_device_id" id="w_device_id" class="form-control input50P" placeholder="<?php echo $SK_BO_Link_Interface[ITS_Lang]?>" >';
