...
if($w == '') {
	// 최대 설치가능한 센서 댓수 제한 8대
	if (current_sensor_count() >= $MAX_numberOfSensor) {
		alert("Limit Over, Install up to $MAX_numberOfSensor sensors.");
		exit;
	}
...
<section id="bo_w" class="container">
    ...
    <div class="tbl_wrap table-responsive">
		...
		<tbody>
		<tr>
			<th scope="row" title="Title"><label for="wr_subject">Name/Serial<strong class="sound_only">필수</strong></label></th>
			<td>
		...
		<tr>
			<th scope="row"><label for="w_sensor_Addr">Sensor<br>IP/Port</label></th>
			<td><input type="text" name="w_sensor_Addr" value="<?php echo $write['w_sensor_Addr'] ?>" id="w_sensor_Addr" required class="form-control input50P required" placeholder="IP Ex:121.165.208.119" size="50"><input type="text" name="w_sensor_Port" value="<?php echo $write['w_sensor_Port'] ?>" id="w_sensor_Port" class="form-control input50P" placeholder="Port Ex:50001" size="50"></td>
		</tr>
		</tbody>
		</table>
		<table class="table table-bordered">
		...
		<tr class="w_hide">
			...
			?>
			<th scope="row" title="Ignore Zone" style="cursor: pointer;" onclick='window.open("<?php echo G5_THEME_URL ?>/utility/status/rlsArea.php?nodeIn=<?php echo $nodeIn ?>&devideID=<?php echo $write['w_device_id']?>&sensorID=<?php echo $write['w_sensor_serial']?>&bo_table=<?php echo $bo_table?>&wr_id=<?php echo $view['wr_id']?>&subject=<?php echo $write['wr_subject']?>","mapsInfo", "width=600,height=400,scrollbars=no");' title="Latitude S"><label for="w_sensor_ignoreZone"><?php echo $SK_BO_Ignore_Area[ITS_Lang]?></label></th>
			<td>
				<!-- input type="text" name="tmp_igZone" id="tmp_igZoneFrX" value="" class="tmpInput" placeholder="Start X" size="4">:
				<input type="text" name="tmp_igZone" id="tmp_igZoneFrY" value="" class="tmpInput" placeholder="Start Y" size="4">~
				<input type="text" name="tmp_igZone" id="tmp_igZoneToX" value="" class="tmpInput" placeholder="End X" size="4">:
				<input type="text" name="tmp_igZone" id="tmp_igZoneToY" value="" class="tmpInput" placeholder="End Y" size="4">
				<input type="button" name="tmp_igZoneAddBtn" id="tmp_igZoneAddBtn" value="Add" class="tmpInput">
				<input type="button" name="tmp_igZoneDelBtn" id="tmp_igZoneDelBtn" value="Del" class="tmpInput" -->
				
				<input type="text" name="w_sensor_ignoreZone" value="<?php echo ($write['w_sensor_ignoreZone']) ?>" id="w_sensor_ignoreZone" class="form-control" placeholder="Ex) 150:2400_190:3000,1500:2800_1700:2900" size="255">
			</td>
		</tr>

		<tr class="w_hide">
			<th scope="row" title="Latitude S"><label for="w_sensor_lat_s"><?php echo $SK_BO_Start_Coord[ITS_Lang]?><br><?php echo $SK_BO_Start[ITS_Lang]?> XY</label></th>
		...
		<tr class="w_hide">
			<th scope="row" title="Latitude E"><label for="w_sensor_lat_e"><?php echo $SK_BO_End_Coord[ITS_Lang]?><br><?php echo $SK_BO_End[ITS_Lang]?> XY</label></th>
		...
		<tr class="w_hide">
			...
			<td><input type="text" name="w_virtual_Addr" value="<?php echo $write['w_virtual_Addr'] ?>" id="w_virtual_Addr" class="form-control input50P" placeholder="IP Ex:121.165.208.119" size="50"><input type="text" name="w_virtual_Port" value="<?php echo $write['w_virtual_Port'] ?>" id="w_virtual_Port" class="form-control input50P" placeholder="Port Ex:50007" size="50"></td>
		</tr>
		<tr class="w_hide">
		...
		<tr class="w_detail_tr">
			...
			<td><input type="text" name="w_url1" value="<?php echo $write['w_url1'] ?>" id="w_url1" class="form-control" placeholder="Ex:http://admin:admin@192.168.0.130/cgi-bin/snapshot.cgi?Channel=0" size="50"></td>
		</tr>
		<tr class="w_detail_tr">
		...
		<tr style="text-align: center;">
			...
			*/ ?>
			<td>Disable<input type="checkbox" class="form-control" name="w_sensor_disable" id="w_sensor_disable" value="1" <?php echo $write[w_sensor_disable]?'checked':'';?> title="Permanently Disable Sensor"></td>
			<?php /*
			...
			*/ ?>
			<td>Apply<input type="checkbox" class="form-control" name="w_sensor_reload" id="w_sensor_reload" value="1" <?php echo $write[w_sensor_reload]?'checked':'';?> title="Restart Sensor when catch first event after save this"></td>
			<?php /*
			...
			<td class="w_default_tr">동일위치고정<input type="checkbox" class="form-control" name="w_event_syncDist" id="w_event_syncDist" value="1" <?php echo $write[w_event_syncDist]?'checked':'';?> title="Allow event that same distence."></td>
			*/ ?>
		</tr>
		</tbody>
		</table>
