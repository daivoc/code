...
function select_w_sensor_model($cur_w_sensor_model, $view=0) {
	$arr = array(
		1 => 'Standalone',
		2 => 'Reflector'
	);
	if($view) {
		$select_w_sensor_model = $arr[$cur_w_sensor_model];
	} else {
		$select_w_sensor_model = '<select name="w_sensor_model" id="w_sensor_model" required class="form-control input50P required" placeholder="<?php echo $SK_BO_Sensor_Type[ITS_Lang]?>" ><option value="Select Type" disabled selected><?php echo $SK_BO_Sensor_Type[ITS_Lang]?></option>';
		while (list($key, $value) = each($arr)) { 
			if($cur_w_sensor_model == $key)
				$select_w_sensor_model .='<option selected value="'.$key.'">'.$value.'</option>';
			else
				$select_w_sensor_model .='<option value="'.$key.'">'.$value.'</option>';
		} 
