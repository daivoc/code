...
<section id="bo_list" class="container">
	...
	<div class="tbl_head01 tbl_wrap table-responsive">
		...
		<tr>
			...
			<th scope="col"><?php echo $SK_BO_Detect_Range[ITS_Lang]?></th>
			<th scope="col"><?php echo $SK_BO_Event_Count[ITS_Lang]?></th>
			<th scope="col">Relay<?php // echo $SK_BO_Sensing_Boundary[ITS_Lang]?></th>
			<th scope="col">Host<?php // echo $SK_BO_Event_Hold[ITS_Lang]?></th>
			<th scope="col">API<?php // echo $SK_BO_Keep_Cycle[ITS_Lang]?></th>
			<th scope="col">Request<?php // echo $SK_BO_Hold_Distance[ITS_Lang]?></th>
		...
		<tr class="w_default_tr">
			...
			<td class="td_board"><?php echo round($list[$i]['w_sensor_ignoreS'] / 1000, 2) ?>m ~ <?php echo round($list[$i]['w_sensor_ignoreE'] / 1000, 2) ?>m</td>
			<td class="td_board"><?php echo $list[$i]['w_event_holdTime'] ?></td>
			<td class="td_board"><?php echo $DEVICE_alert[$list[$i]['w_alert_Port']].'/'.$list[$i]['w_alert_Value'].'s'; ?></td>
			<td class="td_board"><?php echo $list[$i]['w_host_Addr'].'<br>'.$list[$i]['w_host_Addr2'] ; ?></td>
			<td class="td_board"><?php echo explode("||", $list[$i]['wr_8'])[0].'<br>'.explode("||", $list[$i]['wr_9'])[0]; ?></td>
			<td class="td_board"><?php echo explode("/", (explode("||", $list[$i]['wr_4'])[2]))[2].'<br>'.explode("/", (explode("||", $list[$i]['wr_5'])[2]))[2]; ?></td>
			<td class="td_board pointer" onclick='window.open("<?php echo G5_THEME_URL ?>/utility/scheduleBlock/weekView.php?bo_table=<?php echo $bo_table?>&wr_id=<?php echo $list[$i]['wr_id'] ?>&wr_subject=<?php echo $list[$i]['wr_subject']?>&w_sensor_serial=<?php echo $list[$i]['w_sensor_serial']?>","scheduleWeek_RLS_<?php echo $list[$i]['wr_id']?>", "width=600,height=500,scrollbars=no");'><?php echo $SK_BO_Weekly[ITS_Lang]?></td>
			<td class="td_board pointer" onclick='window.open("<?php echo G5_THEME_URL ?>/utility/scheduleBlock/index.php?bo_table=<?php echo $bo_table?>&wr_id=<?php echo $list[$i]['wr_id'] ?>&wr_subject=<?php echo $list[$i]['wr_subject']?>&w_sensor_serial=<?php echo $list[$i]['w_sensor_serial']?>","scheduleBlock_RLS_<?php echo $list[$i]['wr_id']?>", "width=600,height=500,scrollbars=no");'><?php echo $SK_BO_Daily[ITS_Lang]?></td>
			<td class="td_board"><?php echo $list[$i]['w_alarm_disable']; ?></td>
			<?php /*
