...
if($w == '') {
	...
	$write['w_sensor_lat_s'] = 0; // double NOT NULL DEFAULT '0', 37.565289,126.8491215
	...
	$write['w_sensor_ignoreZone'] = $MIN_event_speed; // 상시차단 varchar(256) DEFAULT NULL,
...
.input30P { width: 30%; display: initial; float: left; }
...
$(document).ready(function(){
	...
	$(function() {
		$("#slider-range_w_sensor_ignoreZone").slider({ // 참조 https://jqueryui.com/slider/#rangemax
			range: "max",
			min: <?php echo $MIN_event_speed ?>,
			max: <?php echo $MAX_event_speed ?>,
			value: $("#w_sensor_ignoreZone").val(),
			slide: function(event, ui) {
				$("#w_sensor_ignoreZone").val(ui.value);
				$("#w_sensor_ignoreZoneDisp").text(ui.value);
				$("#w_sensor_ignoreZoneDispSec").text(Math.ceil((ui.value * 0.13) * 10)/10);
			}
		});
		$("#w_sensor_ignoreZoneDisp").text($("#w_sensor_ignoreZone").val());
		$("#w_sensor_ignoreZoneDispSec").text(Math.ceil(($("#w_sensor_ignoreZone").val() * 0.13) * 10)/10);
	});
...
<section id="bo_w" class="container">
    ...
    <div class="tbl_wrap table-responsive">
		...
		<tr class="w_default_tr">
			<th scope="row" title="Limit Due and Level"><label for="w_sensor_angle">Speed Limit<br>Sensing Area</label></th>
			<td>
				<div class="input50P" style="padding: 10px 15px;">
				Speed Limit: <span id="w_sensor_ignoreZoneDisp"></span> km/h
				<input type="text" name="w_sensor_ignoreZone" value="<?php echo $write['w_sensor_ignoreZone'] ?>" id="w_sensor_ignoreZone" readonly class="w_hide">
				<div id="slider-range_w_sensor_ignoreZone"></div>
				</div>
				<div class="input50P" style="padding: 10px 15px;">
				Sensing Area: <span id="w_sensor_scheduleSDisp">0</span> ~ <span id="w_sensor_scheduleEDisp">0</span> M
		...
		<tr class="w_default_tr">
			<th scope="row" title="Limit Due and Level"><label for="w_sensor_angle">Shut Max<br>Speed Filter</label></th>
			<td>
				<div class="input50P" style="padding: 10px 15px;">
				Shut Max: <span id="w_event_holdTimeDisp"></span>
				...
				<div class="input50P" style="padding: 10px 15px;">
				Noise Speed: <span id="w_event_pickTimeDisp"></span> km
		...
		<tr class="w_hide">
			<th scope="row"><label for="w_virtual_Addr" title="Interface information between ITS and sensor (<?php echo $SK_BO_IP[ITS_Lang]?>:<?php echo $SK_BO_Port[ITS_Lang]?>)">Virtual</label></th>
		...
		<tr class="w_hide">
			<th scope="row"><label for="w_sensor_Addr" title="Preset sensor information (<?php echo $SK_BO_IP[ITS_Lang]?>:<?php echo $SK_BO_Port[ITS_Lang]?>)">Sensor</label></th>
		...
		<tr class="w_detail_tr">
			...
			<td><input type="text" name="w_url1" value="<?php echo $write['w_url1'] ?>" id="w_url1" class="form-control" placeholder="URL Info Ex:http://user:pass@ip:port" size="50"></td>
		</tr>
		<tr class="w_detail_tr">
			<th scope="row"><label for="w_url2">Stream</label></th>
			<td><input type="text" name="w_url2" value="<?php echo $write['w_url2'] ?>" id="w_url2" class="form-control" placeholder="URL Info Ex:http://user:pass@ip:port/target" size="50"></td>
		</tr>
		<tr class="w_detail_tr">
		...
		<tr class="w_detail_tr">
			<th scope="row"><label for="w_option" title="Light Control">Brightness</label></th>
			<td>
			<input type="text" name="w_opt11" value="<?php echo $write['w_opt11'] ?>" id="w_opt11" class="form-control input25P" placeholder="HH:MM" size="50">
			<input type="text" name="w_opt12" value="<?php echo $write['w_opt12'] ?>" id="w_opt12" class="form-control input25P" placeholder="80" size="50">
			<input type="text" name="w_opt21" value="<?php echo $write['w_opt21'] ?>" id="w_opt21" class="form-control input25P" placeholder="HH:MM" size="50">
			<input type="text" name="w_opt22" value="<?php echo $write['w_opt22'] ?>" id="w_opt22" class="form-control input25P" placeholder="80" size="50">
			</td>
		</tr>
    ...
    function fwrite_submit(f) {
		...
		var content = 
			...
			+"Speed Limit: "+f.w_sensor_ignoreZone.value+", "
			+"Sensing Area: "+f.w_sensor_scheduleS.value+":"+f.w_sensor_scheduleE.value+", "
			+"Snapshut Count: "+f.w_event_holdTime.value+", "
			+"Noise Filter: "+f.w_event_pickTime.value+", "
