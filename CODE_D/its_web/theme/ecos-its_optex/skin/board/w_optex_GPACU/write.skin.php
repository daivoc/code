...
$(document).ready(function(){
	...
	$('input:checkbox[name^="w_cove"]').click(function () {
		...
		$( "#w_gpacu_cover" ).val(curVal);
		reSetValue(curVal);
	});

	// setCov 값은 최초 실행시 1로 설정 한다.
	var setVal = "<?php echo $write['w_gpacu_group'] ?>";
	setValue(setVal, "w_gpio[]");
	var setCov = "<?php if($write['w_gpacu_cover']) echo $write['w_gpacu_cover']; else echo '11111111'; ?>";
	setValue(setCov, "w_cove[]");
	$( "#w_gpacu_cover" ).val(setCov);
	reSetValue(setCov);
	// setCov
...
<section id="bo_w" class="container">
    ...
    <input type="hidden" name="w_device_id" id="w_device_id" value="<?php echo $write['w_gpacu_group'] ?>">
    <?php
    $option = '';
    $option_hidden = '';
    ...
    <div class="tbl_wrap table-responsive">
		...
		<tr class='w_default_tr'>
			...
			<td>
				<input type="text" name="wr_subject" value="<?php echo $subject ?>" id="wr_subject" required class="form-control input50P required" placeholder="GPACU Name Ex: East Gate 3">
				<input type="text" name="w_gpacu_serial" value="<?php echo $write['w_gpacu_serial'] ?>" id="w_gpacu_serial" readonly class="form-control input50P" placeholder="Serial No" size="50"></td>
			</td>
		</tr>
		<tr class='w_default_tr'>
			<th scope="row" title="Title"><label for="w_gpacu_desc">Desc./Links</label></th>
			<td>
				<input type="text" name="w_gpacu_desc" value="<?php echo $write['w_gpacu_desc'] ?>" id="w_gpacu_desc" class="form-control input50P" placeholder="GPACU Description">
		...
		<tr class='w_default_tr'>
			...
			<td>
				<?php echo select_w_gpacu_status($write['w_gpacu_status']); ?>
				...
				<div class="input25P" style="text-align:center">
					<input class="form-control hide" type="text" name="w_gpacu_cover" value="<?php echo $write['w_gpacu_cover'] ?>" id="w_gpacu_cover">
				...
				<div class="input25P" style="text-align:center">
					<input class="form-control hide" type="text" name="w_gpacu_group" value="<?php echo $write['w_gpacu_group'] ?>" id="w_gpacu_group">
		...
		<tbody>
		<tr class='w_default_tr'>
			<th class='hide' scope="row"><label for="w_gpacu_disable"></label></th>
			<?php // w_gpacu_disable ?>
			<td><?php echo $SK_BO_Disable[ITS_Lang]?><input type="checkbox" class="form-control" name="w_gpacu_disable" id="w_gpacu_disable" value="1" <?php echo $write[w_gpacu_disable]?'checked':'';?> title="Permanently Disable GPACU"></td>
			<?php // w_alarm_disable ?>
    ...
    function fwrite_submit(f) {
		...
		var content = 
			...
			+"gpacu_serial: "+f.w_gpacu_serial.value+", "
			+"gpacu_status: "+f.w_gpacu_status.value+", "
			+"gpacu_cover: "+f.w_gpacu_cover.value+", "
			+"gpacu_group: "+f.w_gpacu_group.value+", "
			+"gpacu_desc: "+f.w_gpacu_desc.value+", "
			...
			+"gpacu_disable: "+is_check('w_gpacu_disable')+", "
