...
<section id="bo_list" class="container">
	...
	<div class="tbl_head01 tbl_wrap table-responsive">
		...
		<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
			...
			<td class="board_CU"><?php echo $list[$i]['w_netapp_model'] ?></td>
			...
			<td class="board_CU"><?php echo $list[$i]['w_netapp_addr'] ?></td>
			<td class="board_CU"><?php echo $list[$i]['w_netapp_serial'] ?></td>
			<!-- td class="board_CU"><?php echo $list[$i]['w_url_01'] ?></td>
			<td class="board_CU"><?php echo $list[$i]['w_url_02'] ?></td -->
			<?php
			if ($list[$i]['w_url_01']) {
				$tmpUrl = "http://".$list[$i]['w_netapp_addr'].$list[$i]['w_url_01'];
				$tmpName = "<font color=green>".$SK_BO_Snapshot[ITS_Lang]."</font>";
			} else if ($list[$i]['w_url_03']) {
				$tmpUrl = $list[$i]['w_url_03'];
				$tmpName = "<font color=orange>".$SK_BO_Snapshot[ITS_Lang]."</font>";
			} else {
				$tmpUrl = "http://".$list[$i]['w_netapp_addr'];
				$tmpName = "<font color=red>".$SK_BO_Snapshot[ITS_Lang]."</font>";
			}
			?>
			<td class="board_CU" onclick='window.open("<?php echo $tmpUrl ?>","Snapshot", "width=600,height=400,scrollbars=no");' style="cursor: pointer;color: green;" ><?php if ($tmpUrl) echo $tmpName ?></td>
			<?php
			if ($list[$i]['w_url_02']) {
				$tmpUrl = "http://".$list[$i]['w_netapp_addr'].$list[$i]['w_url_02'];
				$tmpName = "<font color=green>".$SK_BO_Streaming[ITS_Lang]."</font>";
			} else if ($list[$i]['w_url_04']) {
				$tmpUrl = $list[$i]['w_url_04'];
				$tmpName = "<font color=orange>".$SK_BO_Streaming[ITS_Lang]."</font>";
			} else {
				$tmpUrl = "http://".$list[$i]['w_netapp_addr'];
				$tmpName = "<font color=red>".$SK_BO_Streaming[ITS_Lang]."</font>";
			}
			?>			
			<td class="board_CU" onclick='window.open("<?php echo $tmpUrl ?>","Stream", "width=600,height=400,scrollbars=no");' style="cursor: pointer;color: green;" ><?php if ($tmpUrl) echo $tmpName ?></td>
			<td class="board_CU"><?php echo $list[$i]['wr_id'] ?></td>
