...
$(document).ready(function(){
	...
	$(function() { // 0.1초 부터 $cfg["init_value"]["MAX_event_pickTime"] 까지
	...
	$(function() {
		$("#slider-range_w_event_holdTime").slider({ // 참조 https://jqueryui.com/slider/#rangemax
			...
			slide: function(event, ui) {
				...
				$("#w_event_holdTimeDispSec").text(Math.ceil((ui.value * <?php echo $cfg["init_value"]["Event_read_cycle"] ?>) * 10)/10);
			}
		});
		$("#w_event_holdTimeDisp").text($("#w_event_holdTime").val());
		$("#w_event_holdTimeDispSec").text(Math.ceil(($("#w_event_holdTime").val() * <?php echo $cfg["init_value"]["Event_read_cycle"] ?>) * 10)/10);
	});
