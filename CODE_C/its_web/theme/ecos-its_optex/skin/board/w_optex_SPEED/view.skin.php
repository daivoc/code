...
$(document).ready(function(){
	...
	$('#btn_Monitor').click(function(){
		window.open("http://<?php echo $_SERVER["SERVER_NAME"].":".($view['w_table_PortOut']) ?>","Realtime_Zone_<?php echo $view['w_sensor_serial']?>", "width=500,height=160,scrollbars=no");
		return false;
	});
});
</script>
...
<article id="bo_v" class="container">
	...
	<div class="its_version" style="float:left;color:silver;padding:4px 8px;font-size:8pt;"><?php echo get_w_program_info(G5_CU_PATH_SPEED)[0]; ?></div>
...
*/?>
        <?php if ($view['w_url2']) { ?>
        <input type="button" value="Stream" id="btn_Stream" class="btn btn-success f_right">
		<?php } ?>
        <?php if ($view['w_url1']) { ?>
...
<?php /*
			...
			<td class="w_default_tr">동일거리고정<input type="checkbox" class="form-control" name="w_event_syncDist" id="w_event_syncDist" value="1" <?php echo $view[w_event_syncDist]?'checked':'';?> disabled title="Allow event that same distence."></td>
*/?>
		</tr>
		</tbody>
		</table>
		...
		<tr class="w_default_tr">
			<th scope="row" title="Ignore Zone"><label for="w_sensor_ignoreZone"><?php echo $SK_BO_Speed_Limit[ITS_Lang]?></label></th>
			<td>
				<?php // echo select_w_sensor_ignoreZone($view['w_sensor_ignoreZone'], $MAX_numberOfZone); ?>
				<span id="w_sensor_ignoreZoneDisp"><?php echo $view['w_sensor_ignoreZone'] ?></span> (km/h)
			</td>
		</tr>
		<tr class="w_default_tr">
			<th scope="row" title="Limit Due(Sec)"><label for="w_event_holdTime"><?php echo $SK_BO_Shot_Per_Event[ITS_Lang]?></label></th>
			<td>
		...
		<tr class="w_hide">
			<th scope="row" title="Alarm Level"><label for="w_sensor_spot"><?php echo $SK_BO_Accept_Boundary[ITS_Lang]?></label></th>
			<td>
				<?php echo $view['w_sensor_spot']; ?> Cm
			</td>
		</tr>
		<tr class="w_hide">
			<th scope="row" title="Alarm Level"><label for="w_sensor_offset"><?php echo $SK_BO_Offset[ITS_Lang]?></label></th>
			<td>
				<?php echo $view['w_sensor_offset']; ?> Cm
			</td>
		</tr>
		<tr class="w_default_tr">
