...
<section id="bo_list" class="container">
	...
	<div class="bo_fx">
		...
		<ul class="btn_bo_user">
			...
			<?php if ($write_href && count($list) < 2) { ?><li><a href="<?php echo $write_href ?>" class="btn btn-sm btn-primary"><?php echo $SK_BO_New[ITS_Lang]?></a></li><?php } ?>
		</ul>
		<?php } ?>
	</div>
	<!-- } 게시판 페이지 정보 및 버튼 끝 -->
	...
	<div class="tbl_head01 tbl_wrap table-responsive">
		...
		<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
			...
			<td class="td_board"><?php echo $list[$i]['w_giken_serial'] ?></td>
			<td class="td_board"><?php echo $list[$i]['w_giken_verson'] ?></td>
			<td class="td_board"><?php echo $list[$i]['w_group_level'] ?></td>
			<td class="td_board"><input type="checkbox" name="w_allow_multiple" id="w_allow_multiple" value="1" <?php echo $list[$i][w_allow_multiple]?'checked':'';?> disabled ></td>
			<td class="td_board"><input type="checkbox" name="w_allow_permit" id="w_allow_permit" value="1" <?php echo $list[$i][w_allow_permit]?'checked':'';?> disabled ></td>
			<td class="td_board"><input type="checkbox" name="w_security_mode" id="w_security_mode" value="1" <?php echo $list[$i][w_security_mode]?'checked':'';?> disabled ></td>
			<td class="td_board"><input type="checkbox" name="w_opencv_tuner" id="w_opencv_tuner" value="1" <?php echo $list[$i][w_opencv_tuner]?'checked':'';?> disabled ></td>
			<td class="hide td_board" onclick='window.open("<?php echo G5_THEME_URL ?>/utility/scheduleBlock/weekView.php?bo_table=<?php echo $bo_table?>&wr_id=<?php echo $list[$i]['wr_id']?>&wr_subject=<?php echo $list[$i]['wr_subject']?>&w_sensor_serial=<?php echo $list[$i]['w_sensor_serial']?>","scheduleWeek_<?php echo $list[$i]['wr_id']?>", "width=600,height=500,scrollbars=no");'><?php echo $SK_BO_Weekly[ITS_Lang]?></td>
			<td class="hide td_board" onclick='window.open("<?php echo G5_THEME_URL ?>/utility/scheduleBlock/index.php?bo_table=<?php echo $bo_table?>&wr_id=<?php echo $list[$i]['wr_id']?>&wr_subject=<?php echo $list[$i]['wr_subject']?>&w_sensor_serial=<?php echo $list[$i]['w_sensor_serial']?>","scheduleBlock_<?php echo $list[$i]['wr_id']?>", "width=600,height=500,scrollbars=no");'><?php echo $SK_BO_Daily[ITS_Lang]?></td>
			<td class="td_board"><?php echo $list[$i]['w_stamp'] ?></td>
