...
// python 예: port_out = share["port"]["gikenp"]["portOut"] + int(row['w_device_id'].split('.')[-2])
$monPortNo = G5_CU_GIKENP_PORT_OUT + explode(".", $view['w_device_id'])[2]; // 192.168.[168].30
?>
...
$(document).ready(function(){
	...
	$('#btn_Snapshot').click(function(){
		window.open("http://<?php echo $_SERVER["SERVER_NAME"]."/theme/ecos-its_optex/utility/filemanager/fm.php?ID=its&PATH=data||image||gikenP_".$view['w_sensor_serial'] ?>","Snapshot_Giken_<?php echo $view['w_sensor_serial']?>", "width=500,height=600,scrollbars=no");
		return false;
    });
	...
	$('#btn_Monitor').click(function(){
		window.open("http://<?php echo $_SERVER["SERVER_NAME"].":".$monPortNo ?>","Realtime_Giken_<?php echo $view['w_sensor_serial']?>", "width=360,height=400,scrollbars=no");
		return false;
    });
...
function restartMyself() { // 실행 않됨 kill 권한 문제 인듣 ..
	...
	if (r == true) {
		$.ajax({
			url :  "<?php echo G5_THEME_URL ?>/utility/systemExec/restartGIKENP.php",
		}).done(function(data) {
			console.log(data);
			alert(data);
		});
