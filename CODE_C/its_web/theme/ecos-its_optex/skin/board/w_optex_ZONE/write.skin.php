...
<section id="bo_w" class="container">
    ...
    <div class="tbl_wrap table-responsive">
		...
		<table class="table table-bordered">
		<tbody>
		<tr><?php // wr_subject w_zone_model ?>
			<th scope="row" title="Title"><label for="wr_subject"><?php echo $SK_BO_Name[ITS_Lang]?>/Model</label></th>
			<td>
				...
				<?php echo select_w_zone_model($write['w_zone_model']); ?>
			</td>
		</tr>
		<tr><?php // wr_subject w_zone_model ?>
			<th scope="row" title="Title"><label for="wr_model">Map ID /<br>Serial</label></th>
			<td>
				...
				<input type="text" name="w_zone_serial" value="<?php echo $write['w_zone_serial'] ?>" id="w_zone_serial" readonly class="form-control input50P" placeholder="Serial No" size="50"></td>
			</td>
		</tr>
		<tr><?php // w_zone_desc  ?>
			<th scope="row" title="Title"><label for="w_zone_desc">Description</label></th>
			<td>
				<input type="text" name="w_zone_desc" value="<?php echo $write['w_zone_desc'] ?>" id="w_zone_desc" class="form-control input100P" size="50" maxlength="255" placeholder="Sensor Description">
			</td>
		</tr>
		</tbody>
		</table>
		...
		<tr ><?php // w_sns_ ?>
			...
			<td>
			<input type="text" name="w_sns_0" value="<?php echo $write['w_sns_0'] ?>" id="w_sns_0" required class="form-control input50P required" placeholder="Sensor ID" size="50">
			<input type="text" name="w_sns_1" value="<?php echo $write['w_sns_1'] ?>" id="w_sns_1" class="hide form-control input50P" placeholder="Sensor ID" size="50">
			<input type="text" name="w_sns_2" value="<?php echo $write['w_sns_2'] ?>" id="w_sns_2" class="hide form-control input50P" placeholder="Sensor ID" size="50">
			<input type="text" name="w_sns_3" value="<?php echo $write['w_sns_3'] ?>" id="w_sns_3" class="hide form-control input50P" placeholder="Sensor ID" size="50">
		...
		<tr><?php // w_cam_ ?>
			...
			<input type="text" name="w_ptz_0" value="<?php echo $write['w_ptz_0'] ?>" id="w_ptz_0" class="form-control input20P" placeholder="PTZ Value" size="50">
			<input type="checkbox" class="form-control input5P" name="w_ptzOn_0" id="w_ptzOn_0" value="1" <?php echo $write[w_ptzOn_0]?'checked':'';?>>
			<?php echo select_w_camera($write['w_cam_1'], 'w_cam_1'); ?>
			<input type="text" name="w_ptz_1" value="<?php echo $write['w_ptz_1'] ?>" id="w_ptz_1" class="form-control input20P" placeholder="PTZ Value" size="50">
			<input type="checkbox" class="form-control input5P" name="w_ptzOn_1" id="w_ptzOn_1" value="1" <?php echo $write[w_ptzOn_1]?'checked':'';?>>
			<?php echo select_w_camera($write['w_cam_2'], 'w_cam_2'); ?>
			<input type="text" name="w_ptz_2" value="<?php echo $write['w_ptz_2'] ?>" id="w_ptz_2" class="form-control input20P" placeholder="PTZ Value" size="50">
			<input type="checkbox" class="form-control input5P" name="w_ptzOn_2" id="w_ptzOn_2" value="1" <?php echo $write[w_ptzOn_2]?'checked':'';?>>
			<?php echo select_w_camera($write['w_cam_3'], 'w_cam_3'); ?>
			<input type="text" name="w_ptz_3" value="<?php echo $write['w_ptz_3'] ?>" id="w_ptz_3" class="form-control input20P" placeholder="PTZ Value" size="50">
			<input type="checkbox" class="form-control input5P" name="w_ptzOn_3" id="w_ptzOn_3" value="1" <?php echo $write[w_ptzOn_3]?'checked':'';?>>
			</td>
		</tr>
		</tbody>
		</table>
		...
		<tbody>
		<tr>
			<th scope="row"><label for="w_zone_disable"></label></th>
			<?php // w_zone_disable ?>
			<td><?php echo $SK_BO_Disable[ITS_Lang]?><input type="checkbox" class="form-control" name="w_zone_disable" id="w_zone_disable" value="1" <?php echo $write[w_zone_disable]?'checked':'';?> title="Permanently Disable Sensor"></td>
			<?php // w_alarm_disable ?>
	...
	$(document).ready(function() {
		...
		$("[id^=w_cam]").change(function(){
			var selected = $(this).find('option:selected');
			var auth = selected.data('auth'); 
			var myID = $(this).attr("id").replace( 'w_cam', 'w_ptz' );
			var camInfo = auth.split([',']);
			
			// 카메라 접속 정보를 ajax를 통해 현재 연결된 카메라의 PTZ 값을 w_ptz_?에 입력한다.
			$.ajax({
				type: 'POST',
				url: '<?php echo "$board_skin_url/ajaxPTZ.php" ?>',
				data: { camInfo:camInfo },
				success: function (data) {
					if(data) {
						var camInfo = data.split("\n");
						if(camInfo[0]) {
							var valPTZ = camInfo[0].trim().split(' ').join(',');
							$("#"+myID).val(valPTZ);
						} else {
							$("#"+myID).val('');
						}
    ...
    function fwrite_submit(f) {
		...
		var content = 
			...
			+"sensor_serial: "+f.w_zone_serial.value+", "
			+"sensor_model: "+f.w_zone_model.value+", "
			+"sensor_desc: "+f.w_zone_desc.value+", "
			+"sns_0: "+f.w_sns_0.value+", "
			+"sns_1: "+f.w_sns_1.value+", "
			+"sns_2: "+f.w_sns_2.value+", "
			+"sns_3: "+f.w_sns_3.value+", "
			...
			+"box_id: "+f.w_box_id.value+", "
			+"license: "+f.w_license.value+", "
			+"sensor_disable: "+is_check('w_zone_disable')+", "
