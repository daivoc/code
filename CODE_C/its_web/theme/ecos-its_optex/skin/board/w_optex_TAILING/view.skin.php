...
$(document).ready(function(){
	...
	$('#btn_Snapshot').click(function(){
		window.open("http://<?php echo $_SERVER["SERVER_NAME"]."/theme/ecos-its_optex/utility/filemanager/fm.php?ID=its&PATH=data||image||tailing_".$view['w_sensor_serial'] ?>","Snapshot_Tailing_<?php echo $view['w_sensor_serial']?>", "width=500,height=600,scrollbars=no");
		return false;
    });
	...
	$('#btn_Monitor').click(function(){
		// define('G5_CU_TAILING_PORT_IN', 34000); // TAILING 모니터링을 위한 TCP Port 
		// define('G5_CU_TAILING_PORT_OUT', 34100); // TAILING 모니터링을 위한 TCP Port 
		window.open("http://<?php echo $_SERVER["SERVER_NAME"].":".(G5_CU_TAILING_PORT_OUT + $view['wr_id']) ?>","Realtime_Tailing_<?php echo $view['w_sensor_serial']?>", "width=360,height=920,scrollbars=no");
		return false;
    });
	$('#btn_Setup').click(function(){
		window.open("http://<?php echo $_SERVER["SERVER_NAME"] ?>:16830","Realtime_Tailing_Setup", "width=640, height=480,scrollbars=no");
		return false;
	});
...
function restartMyself() { // 실행 않됨 kill 권한 문제 인듣 ..
	...
	if (r == true) {
		$.ajax({
			url :  "<?php echo G5_THEME_URL ?>/utility/systemExec/restartTAILING.php",
		}).done(function(data) {
			console.log(data);
			alert(data);
		});
...
<article id="bo_v" class="container">
	...
	<div>
		...
		<?php if ($view['w_opencv_tuner']) { // 조정모드(?>
	...
	<section id="bo_v_atc" class="">
		...
		<tr class='w_detail_tr'>
			<th scope="row"><label for="w_sensor_url">Sensor URL</label></th>
			<td>
			<input type="text" name="w_sensor_url" value="<?php echo $view['w_sensor_url'] ?>" id="w_sensor_url" class="form-control input50P" placeholder="http, rtsp, rtp, stream file" size="50">
			</td>
		</tr>
		<tr class='w_default_tr'>
		...
		<tr class='w_detail_tr'>
			...
			<span class='hide' style="display:inline-grid;padding: 0 10px;vertical-align: top;"><label style="color:crimson;margin: unset;">Use Mask</label><input type="checkbox" class="form-control" name="w_opencv_mask" id="w_opencv_mask" value="1" <?php echo $view[w_opencv_mask]?'checked':'';?> disabled ></span>
			</td>
		</tr>
		<tr class='w_detail_tr'>
		...
		<tr class='w_detail_tr'>
			<th scope="row"><label for="w_opencv">Filter Limit</label></th>
			<td>
			...
			<div class="inputPart">Timeout:<br><input type="text" name="w_reset_interval" value="<?php echo $view['w_reset_interval'] ?>" id="w_reset_interval" readonly class="form-control"></div>
			</td>
		</tr>
		<tr class='w_detail_tr'>
			<th scope="row"><label for="w_alert_Port">Tailing</label></th>
			<td>
			<div class="inputPart">Width Start:<br><input type="text" name="w_opencv_tail_x" value="<?php echo $view['w_opencv_tail_x'] ?>" id="w_opencv_tail_x" readonly class="form-control" placeholder="w_opencv_tail_x" size="50"></div>
			<div class="inputPart">Width End:<br><input type="text" name="w_opencv_tail_w" value="<?php echo $view['w_opencv_tail_w'] ?>" id="w_opencv_tail_w" readonly class="form-control" placeholder="w_opencv_tail_w" size="50"></div>
			<div class="inputPart">Height End:<br><input type="text" name="w_opencv_tail_y" value="<?php echo $view['w_opencv_tail_y'] ?>" id="w_opencv_tail_y" readonly class="form-control" placeholder="w_opencv_tail_y" size="50"></div>
			<div class="inputPart">Height End:<br><input type="text" name="w_opencv_tail_h" value="<?php echo $view['w_opencv_tail_h'] ?>" id="w_opencv_tail_h" readonly class="form-control" placeholder="w_opencv_tail_h" size="50"></div>
			<div style="clear: both;"></div>
			<div class="inputPart">Limit Width:<br><input type="text" name="w_opencv_tail_over_x" value="<?php echo $view['w_opencv_tail_over_x'] ?>" id="w_opencv_tail_over_x" readonly class="form-control" placeholder="w_opencv_tail_over_x" size="50"></div>
			<div class="inputPart">Limit Height:<br><input type="text" name="w_opencv_tail_over_y" value="<?php echo $view['w_opencv_tail_over_y'] ?>" id="w_opencv_tail_over_y" readonly class="form-control" placeholder="w_opencv_tail_over_y" size="50"></div>
			<div class="inputPart">Limit Pixels:<br><input type="text" name="w_opencv_tail_pixel" value="<?php echo $view['w_opencv_tail_pixel'] ?>" id="w_opencv_tail_pixel" readonly class="form-control" placeholder="w_opencv_tail_pixel" size="50"></div>
			</td>
		</tr>

		<tr class="w_detail_tr">
			<th scope="row"><label for="w_opencv_filter">Pre Process</label></th>
			<td>
			...
			<span style="display:inline-grid;padding: 0 10px;vertical-align: top;"><label style="color:crimson;margin: unset;">Tuner Mode</label><input type="checkbox" class="form-control" name="w_opencv_tuner" id="w_opencv_tuner" value="1" <?php echo $write[w_opencv_tuner]?'checked':'';?> disabled ></span>
		...
		<tr class="w_detail_tr">
			<th scope="row"><label for="w_alert_Port">Relay<br>Output/Time</label></th>
			<td>
			<div class="inputPart">Door Open:<br><input type="text" name="w_alert_Value1" value="<?php echo $view['w_alert_Value1'] ?>" id="w_alert_Value1" readonly class="form-control" size="50"></div>
			<div class="inputPart">Anti Denial:<br><input type="text" name="w_alert_Value2" value="<?php echo $view['w_alert_Value2'] ?>" id="w_alert_Value2" readonly class="form-control" size="50"></div>
			<div class="inputPart">Anti Tailing:<br><input type="text" name="w_alert_Value3" value="<?php echo $view['w_alert_Value3'] ?>" id="w_alert_Value3" readonly class="form-control" size="50"></div>
			<div class="inputPart">Alarm Event:<br><input type="text" name="w_alert_Value4" value="<?php echo $view['w_alert_Value4'] ?>" id="w_alert_Value4" readonly class="form-control" size="50"></div>
			</td>
		</tr>
