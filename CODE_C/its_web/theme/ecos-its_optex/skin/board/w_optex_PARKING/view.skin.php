...
#select_w_sensor_ignoreZone li { margin: 1px; padding: 1px; float: left; width: 32px; height: 22px; border-radius: 16px; font-size: 6pt; text-align: center; }
...
#select_w_sensor_scheduleZone li { margin: 1px; padding: 1px; float: left; width: 32px; height: 22px; border-radius: 16px; font-size: 6pt; text-align: center; }
...
$(document).ready(function(){
	...
	$('#btn_Monitor').click(function(){
		window.open("http://<?php echo $_SERVER["SERVER_NAME"].":".($view['w_table_PortOut']) ?>","Realtime_Zone_<?php echo $view['w_sensor_serial']?>", "width=530,height=160,scrollbars=no");
		return false;
	});
});
</script>
...
<article id="bo_v" class="container">
	...
	<div class="its_version" style="float:left;color:silver;padding:4px 8px;font-size:8pt;"><?php echo get_w_program_info(G5_CU_PATH_PARKING)[0]; ?></div>
    ...
    <section id="bo_v_atc" class="col-md-10 col-md-offset-1">
		...
		<tr>
			...
			<td class="w_hide"><?php echo $SK_BO_Disable[ITS_Lang]?><input type="checkbox" class="form-control" name="w_sensor_disable" id="w_sensor_disable" value="1" <?php echo $view[w_sensor_disable]?'checked':'';?> disabled title="Permanently Disable Sensor"></td>
			...
			<td class="w_hide"><?php echo $SK_BO_Stop_Alarm[ITS_Lang]?><input type="checkbox" class="form-control" name="w_alarm_disable" id="w_alarm_disable" value="1" <?php echo $view[w_alarm_disable]?'checked':'';?> disabled title="Do not send valid event to host."></td>
			...
			<td class="w_default_tr w_hide">허용주기고정<input type="checkbox" class="form-control" name="w_event_keepHole" id="w_event_keepHole" value="1"<?php echo $view[w_event_keepHole]?'checked':'';?> disabled title="Keep event hold cycle."></td>
			<td class="w_default_tr w_hide">동일위치고정<input type="checkbox" class="form-control" name="w_event_syncDist" id="w_event_syncDist" value="1" <?php echo $view[w_event_syncDist]?'checked':'';?> disabled title="Allow event that same distence."></td>
		</tr>
		</tbody>
		</table>
		<table class="table table-bordered">
		...
		<tr class="w_hide">
			<th scope="row" title="Ignore Zone"><label for="w_sensor_ignoreZone">차단지역</label></th>
			<td>
				<?php echo select_w_sensor_ignoreZone($view['w_sensor_ignoreZone'], $MAX_numberOfZone); ?>
			</td>
		</tr>
		<tr class="w_hide">
			<th scope="row" title="Limit Due(Sec)"><label for="w_event_holdTime"><?php echo $SK_BO_Event_Count[ITS_Lang]?></label></th>
			<td>
		...
		<tr class="w_default_tr">
			<th scope="row" title="Sensor Face"><label for="w_sensor_face">Face</label></th>
			<td>
				<?php echo select_w_sensor_face($view['w_sensor_face'], 1); ?>
			</td>
		</tr>
		<tr class="w_default_tr">
			<th scope="row" title="Sensor Face"><label for="w_capacity_max">Maxmum</label></th>
			<td>
				<?php echo $view['w_capacity_max']; ?>
			</td>
		</tr>
		<tr class="w_default_tr">
			<th scope="row" title="Sensor Face"><label for="w_capacity_cur">Current</label></th>
			<td>
				<?php echo $view['w_capacity_cur']; ?>
			</td>
		</tr>
		<tr class="w_hide">
			<th scope="row" title="Sensor Angle"><label for="w_sensor_angle">Angle</label></th>
			<td>
				<span id="w_sensor_angleDisp"><?php echo $view['w_sensor_angle'] ?></span> 
			</td>
		</tr>
		<tr class="w_hide">
		...
		<tr class="w_detail_tr">
			<th scope="row"><label for="w_master_Addr">Sensor2</label></th>
			<td>
				<?php echo $view['w_sensor_Addr2'] ?> : <?php echo $view['w_sensor_Port2'] ?>
			</td>
		</tr>
		<tr class="w_hide">
		...
		<tr class="w_detail_tr">
			<th scope="row"><label for="w_neighbors_ip">Neighbors</label></th>
			<td>
				<?php echo $view['w_neighbors_ip'] ?>
			</td>
		</tr>
		<tr class="w_detail_tr">
			<th scope="row"><label for="w_alert">Alert</label></th>
			<td>
				<?php echo $DEVICE_alert[$view['w_alert_Port']] ?> : <?php echo $view['w_alert_Value'] ?> Sec
			</td>
		</tr>
		<tr class="w_hide">
