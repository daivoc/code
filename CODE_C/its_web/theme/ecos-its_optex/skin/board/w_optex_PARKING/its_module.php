...
function get_w_virtual_Addr($cur_w_device_id) { 
	if($cur_w_device_id) {
...
function get_w_sensor_serial($bo_table, $w_system_ip, $wr_id) { // MD5(Wits IP + Sensor ID) $w_system_ip, $w_device_id
	...
	$string =  preg_replace('/[^A-Za-z0-9_]/', '_', $string); // Removes special chars.
...
function select_w_sensor_model($cur_w_sensor_model) {
	...
	// $select_w_sensor_model = '<select name="w_sensor_model" id="w_sensor_model" required class="form-control input50P required" placeholder="Sensor Model" ><option value="" disabled selected>Model</option>';
	$select_w_sensor_model = '<select name="w_sensor_model" id="w_sensor_model" required class="form-control input50P required" placeholder="Sensor Model" >';
...
function select_w_sensor_face($cur_w_sensor_face, $view=0) {
	...
	if($view) {
		...
		$select_w_sensor_face = '<select name="w_sensor_face" id="w_sensor_face" class="form-control input50P" title="Direction" >';
...
function select_w_sensor_ignoreZone($w_sensor_ignoreZone, $numberOfZone=8) { // BSS 시리즈 비활성 영역 설정
	...
	for ($i=0; $i<$numberOfZone; $i++) {
		if(in_array("$i", $cur_w_sensor_ignoreZone))
			$select_w_sensor_ignoreZone .= '<li class="ui-state-default ui-selected" id="igZone_'.$i.'">'.$RLS_map_LA[$i].'</li>';
		else
			$select_w_sensor_ignoreZone .= '<li class="ui-state-default" id="igZone_'.$i.'">'.$RLS_map_LA[$i].'</li>';
	}
	$select_w_sensor_ignoreZone .= '</ol>';
	return $select_w_sensor_ignoreZone;
}
function select_w_sensor_scheduleZone($w_sensor_scheduleZone, $numberOfZone=8) { // BSS 시리즈 비활성 영역 설정
	// 가변 변수로 함수나 클래스 메쏘드 안에서 PHP 자동 전역 배열을 사용할 수 없음에 주의하십시오. $this 변수도 특수 변수로써, 동적으로 참조할 수 없습니다.
	$RLS_map_LA[4] = 'A11'; // ['A11'] = 4
	$RLS_map_LA[5] = 'A12'; // ['A12'] = 5
	$RLS_map_LA[6] = 'A21'; // ['A21'] = 6
	...
	for ($i=0; $i<$numberOfZone; $i++) {
		if(in_array("$i", $cur_w_sensor_scheduleZone))
			$select_w_sensor_scheduleZone .= '<li class="ui-state-default ui-selected" id="igZone_'.$i.'">'.$RLS_map_LA[$i].'</li>';
		else
			$select_w_sensor_scheduleZone .= '<li class="ui-state-default" id="igZone_'.$i.'">'.$RLS_map_LA[$i].'</li>';
	}
