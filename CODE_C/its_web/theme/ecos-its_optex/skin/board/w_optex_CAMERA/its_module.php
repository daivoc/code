...
function get_w_camera_serial($bo_table, $w_system_ip, $wr_id) { // MD5(Wits IP + Sensor ID) $w_system_ip
	...
	// $get_w_camera_serial =  strtoupper($string);
	return $get_w_camera_serial;
}
...
function select_w_camera_model($cur_w_camera_model, $icc) {
	// $icc = json_decode(file_get_contents('/home/pi/MONITOR/camera.json', true), true);
	$select_w_camera_model = '<select name="w_camera_model" id="w_camera_model" required class="form-control input25P required" placeholder="<?php echo $SK_BO_Model_Name[ITS_Lang]?>" ><option></option>';
	foreach($icc as $key => $item){
		if($cur_w_camera_model == $key)
			$select_w_camera_model .='<option selected>'.$key.'</option>';
		else
			$select_w_camera_model .='<option>'.$key.'</option>';
	}
	$select_w_camera_model .= '</select>';
	return $select_w_camera_model;

	// foreach ($CAMERA_model as $value) {
		// if($cur_w_camera_model == $value)
			// $select_w_camera_model .='<option selected>'.$value.'</option>';
		// else
			// $select_w_camera_model .='<option>'.$value.'</option>';
	// }
	// $select_w_camera_model .= '</select>';
	// return $select_w_camera_model;
}
