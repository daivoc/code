...
var portIn = cfg["port"]["forZone"];
var portOut = cfg["port"]["mainZone"];
...
io.sockets.on('connection', function (socket) { // 내부 Client로 부터 받은 작업 실행
	...
	socket.on('setTrig', function(data) { // SVG View Point 관련
		// data.level, data.hold.cont, data.hold.cont, data.hold.cont, data.size.max, data.size.max
		cfg.trigger=data.trigger;
		saveConfigJson(cfg.path.data+"/trigger.json", cfg.trigger);
		saveConfigJson('config_RLS3.json', cfg);
	});
	socket.on('readTrig', function() { // 이벤트 등급값을 클라이언트로 전송()
		io.sockets.emit('readTrig', cfg.trigger); // 클라이언트동기화 
	});
