...
def Save_Config(data):
	...
	config["sensor"]["pickupDue"] = data["sensor_pickupDue"]
	config["sensor"]["heartBeatDue"] = data["sensor_heartBeatDue"]
	if data["sensor_addr"] == "192.168.168.30":
		config["sensor"]["masquerade"] = True
	else:
		config["sensor"]["masquerade"] = False
	config["server"]["ims1st"]["addr"] = data["server_ims1st_addr"]
	config["server"]["ims2nd"]["addr"] = data["server_ims2nd_addr"]

	config["server"]["user1st"]["addr"] = data["server_user1st_addr"]
	config["server"]["user1st"]["port"] = data["server_user1st_port"]
	config["server"]["user1st"]["method"] = data["server_user1st_method"]
	config["server"]["user1st"]["id"] = data["server_user1st_id"]
	config["server"]["user1st"]["password"] = data["server_user1st_password"]
	config["server"]["user1st"]["command"] = data["server_user1st_command"]

	config["server"]["user2nd"]["addr"] = data["server_user2nd_addr"]
	config["server"]["user2nd"]["port"] = data["server_user2nd_port"]
	config["server"]["user2nd"]["method"] = data["server_user2nd_method"]
	config["server"]["user2nd"]["id"] = data["server_user2nd_id"]
	config["server"]["user2nd"]["password"] = data["server_user2nd_password"]
	config["server"]["user2nd"]["command"] = data["server_user2nd_command"]
...
def Is_Port_Open(port):
	...
	if result_of_check == 0: 
		...
		status = False
	emit('Return_Is_Port_Open', {"status":status, "port":port})
	return status
...
def main():
	...
	config["sensor"]["vServer"] = masquerade()
	saveConfig(config, "./config.json")
	print(f'Access: http://localhost:{config["port"]["mainSetup"]}')
	socketio.run(app, host='0.0.0.0', port=config["port"]["mainSetup"])

if __name__ == '__main__':
	config = readConfig("./config.json")
	config["itsEventProtocol"] = readConfig(f'{config["path"]["itsAPI3"]}/itsEventProtocol.json')
	main()