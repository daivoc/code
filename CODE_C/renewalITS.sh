...
# find . -maxdepth 1 -type d \( ! -name . \) -exec bash -c 'cd {}; test -e node_modules && pwd;' \;
# find . -maxdepth 1 -type d \( ! -name . \) -exec bash -c 'cd {}; test -e node_modules && rm -rf node_modules && ln -sf ../node_modules .;'\;
if [[ $ModsNode =~ [Yy]$ ]]; then
    for itsIP in $args; do
