...
def kill_demon_GIKENP():
	# 아래 grep에서 스페이스를 포함해야 자신(run_GIKENP.pyc)을 죽이지 않는다
	# cmd = "pkill -9 -ef $(ps aux | grep '/gikenp_' | awk '{print $2}')"
	cmd = "pkill -9 -ef GIKENP/GIKENP 2>&1" ## 주의) 직전에 실행된 프로세서를 죽이는 오류 발생 가능
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return "kill_demon_GIKENP"
...
def run_demon_GIKENP_PY(cfgJson): 
	cmd = "python %s/GIKENP.pyc %s 2>&1 & " % (share['path']['gikenp'],cfgJson)
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p
...
def read_table_w_cfg_gikenp(): ## 사용자 환경 변수 로딩
	query = "SELECT * FROM " + share['table']['gikenp'] + " WHERE w_sensor_disable = 0" + " ORDER BY wr_id DESC" 
	try:
...
def main():
	tableRow = read_table_w_cfg_gikenp()
	...
	for row in tableRow:
		...
		owner['user_table'] = {}
		owner['user_table']['board']    = share['table']['gikenp']
		...
		owner['interface']['port_JS_in']   = share['port']['gikenp']['portIn'] + int(row['w_device_id'].split('.')[-2])
		owner['interface']['port_JS_out']  = share['port']['gikenp']['portOut'] + int(row['w_device_id'].split('.')[-2])
		owner['interface']['port_PY_in']   = share['port']['gikenp']['portIn'] + int(row['w_device_id'].split('.')[-2]) + 1
		owner['interface']['port_PY_out']  = share['port']['gikenp']['portOut'] + int(row['w_device_id'].split('.')[-2]) + 1
		owner['interface']['ip']           = row['w_device_id'].split('_')[1]

		owner['file'] = {}
		owner['file']['conf_common']    = '/home/pi/common/config.json'
		owner['file']['html_source']    = share['path']['gikenp']+'/GIKENP.html'
		owner['file']['html_target']    = share['path']['gikenp']+'/index_'+str(owner['interface']['port_JS_in'])+'.html'
		owner['file']['conf_gikenp']    = share['path']['gikenp']+'/gikenp_'+str(owner['interface']['port_JS_in'])+'.json'

		owner['file']['log_gikenp']     = share['path']['log']+'/'+row['w_sensor_serial']+'.log'

		owner['file']['image_folder']   = share['path']['img']+'/gikenP_'+row['w_sensor_serial']
		# owner['file']['image_NIR']      = share['path']['img']+'/gikenP_'+row['w_sensor_serial']+'/NIR'
		owner['file']['image_tailing']  = share['path']['img']+'/gikenP_'+row['w_sensor_serial']+'/tailing'
		owner['file']['image_manual']   = share['path']['img']+'/gikenP_'+row['w_sensor_serial']+'/manual'
		owner['file']['image_unknown']  = share['path']['img']+'/gikenP_'+row['w_sensor_serial']+'/unknown'
		owner['file']['image_final']    = share['path']['img']+'/gikenP_'+row['w_sensor_serial']+'/final'
		owner['file']['image_base']     = '0_img.png'
		owner['file']['image_live']     = '1_img.png'
		owner['file']['image_sync']     = '9_img.png'

		try:
			os.makedirs(owner['file']['img'])
			os.chmod(owner['file']['img'], 0o707)
		except:
		...
		owner['opencv']['tail_w']       = 320 # row['w_opencv_tail_w']
		owner['opencv']['tail_h']       = 240 # row['w_opencv_tail_h']
		owner['opencv']['tail_x']       = 0   # row['w_opencv_tail_x']
		owner['opencv']['tail_y']       = 0   # row['w_opencv_tail_y']
		...
		owner['log_table']['tbl_live']  = 'w_log_gikenP_live_'+row['w_giken_serial']
		owner['log_table']['tbl_min']   = 'w_log_gikenP_min_'+row['w_giken_serial']
		owner['log_table']['tbl_hour']  = 'w_log_gikenP_hour_'+row['w_giken_serial']
		owner['log_table']['tbl_day']   = 'w_log_gikenP_day_'+row['w_giken_serial']
		owner['log_table']['tbl_week']  = 'w_log_gikenP_week_'+row['w_giken_serial']
		owner['log_table']['tbl_month'] = 'w_log_gikenP_month_'+row['w_giken_serial']
		owner['log_table']['tbl_sum']   = 'w_log_gikenP_sum_'+row['w_giken_serial']
		...
		owner['area']['timeover'] = {}
		owner['area']['timeover']['inner'] = float(row['w_reset_interval'])
		owner['area']['timeover']['outer'] = float(row['w_reset_interval'])

		owner['gpio'] = {}
		if share['gpio']['activeOn']: # NO Mode
			owner['gpio']['swOn'] = 1 # NO 인경우
			owner['gpio']['swOff'] = 0 # NO 인경우
		else: # NC Mode
			owner['gpio']['swOn'] = 0 # NC 인경우
			owner['gpio']['swOff'] = 1 # NC 인경우
		...
		MASQUERADE(1,owner['interface']['port_MQ']) # 강제 실행
		...
		# socketIO - TCP 포트 통신
		owner['server']['socket'] = {}
		owner['server']['socket']['flag'] = {}
		owner['server']['socket']['flag']['P'] = False
		owner['server']['socket']['flag']['S'] = False
		try:
			...
			if addr and port and value:
				owner['server']['socket']['P'] = {
					"addr":addr,
					"port":port,
					"value":value
				}
				owner['server']['socket']['flag']['P'] = True
		except:
			pass
		...
		try:
			...
			value = elements[2]
			if addr and port and value:
				owner['server']['socket']['S'] = {
					"addr":addr,
					"port":port,
					"value":value
				}
				owner['server']['socket']['flag']['S'] = True
		except:
			pass
		...
		# 메인 프로그램 실행
		run_demon_GIKENP_PY(owner['file']['conf_gikenp'])
