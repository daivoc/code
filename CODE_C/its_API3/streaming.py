...
$ sudo apt-get install libatlas-base-dev ## https://stackoverflow.com/questions/49799798/can-not-import-opencv-in-python3-in-raspberry-pi3
...
실시간 이미지는 /var/www/html/its_web/theme/ecos-its_optex/user/image/dirCur 에 저장 된다.
...
def makeAVI(path,name):
	...
	frameRate = 1 * cfg["camera"]["holeTime"]
	...
	out = cv2.VideoWriter('{}/{}.avi'.format(path,name),cv2.VideoWriter_fourcc(*'DIVX'), frameRate, size) # Good
...
if __name__ == '__main__':
	...
	streaming = readConfig('/home/pi/API3/streaming.json')

	cfg = {}
	cfg["runItsAPI"] = 0
	if os.path.isfile('/home/pi/API3/itsAPI.json'): # itsAPI.py로 부터 실행시
		itsAPI = readConfig('/home/pi/API3/itsAPI.json')
