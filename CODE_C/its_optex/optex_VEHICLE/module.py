...
config = readConfig('/home/pi/optex_VEHICLE/config.json')
...
def database_test(): # Optex Microwave
	try:
		conn = MySQLdb.connect(host=config["mysql"]["host"], user=config["mysql"]["user"], passwd=config["mysql"]["pass"], db=config["mysql"]["name"], charset='utf8', use_unicode=True) 
...
def kill_demon_check_VEHICLE(): 
	cmd = "pkill -9 -ef optex_VEHICLE.pyc 2>&1" 
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
...
# 확인된 변수로 데몬을 실행 한다
def run_demon_check_VEHICLE(arg): # python -W ignore
	cmd = "python /home/pi/optex_VEHICLE/optex_VEHICLE.pyc %s 2>&1 & " % arg
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)

	# ## ## optex_RLS_alarm 동시 실행
	# cmd = "python /home/pi/optex_VEHICLE/optex_RLS_alarm.pyc %s 2>&1 & " % arg
	# p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)

# 실행하고 있는 arg 단어가 포함된 데몬을 awk를 이용하여 모두 종료 시킨다.
def kill_demon_realtime_VEHICLE(): 
...
# 실행하고 있는 arg 단어가 포함된 데몬을 awk를 이용하여 모두 종료 시킨다.
def run_demon_realtime_VEHICLE(arg): 
	cmd = "cd /home/pi/optex_VEHICLE; node realtime_RLS.js %s 2>&1 & " % (arg)
	# print cmd
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
	return p
...
def restart_its(): 
	cmd = "python /home/pi/optex_VEHICLE/run_optex.pyc"
	print(cmd)
	p = subprocess.Popen(cmd, shell=True, stderr=subprocess.PIPE)
...
def create_table_RLS_RAW(tableName=''): # 테이블 생성 ITS_VEHICLE_row_table = "w_log_RLS_RAW"
	try:
		...
		tbl_w_log_sensor_sql = """
			...
			""" % (config["table"]["vechile"], tableName)
...
def insert_event_RLS_RAW(tableName, w_evt_id=0, w_evt_X=0, w_evt_Y=0, w_evt_S=0, w_evt_zone=''): 
	query = "INSERT INTO "+config["table"]["vechile"]+"_"+tableName+"(w_evt_id, w_evt_X, w_evt_Y, w_evt_S, w_evt_zone) VALUES(%s, %s, %s, %s, %s)"
...
# def create_table_w_log_VEHICLE(postfix=''): # create_table_w_log(uniqueOfSensor)
# 	try:
...
# def insert_event_VEHICLE(tableName, w_evt_id=0, w_evt_X=0, w_evt_Y=0, w_evt_S=0, w_evt_zone=''): 
...
def read_table_w_cfg_sensorID(): ###################### Optex REDSCAN
	query = "SELECT * FROM " + config["table"]["rls_r"] + " WHERE w_sensor_disable = 0" + " ORDER BY wr_id DESC" 
	try:
		conn = MySQLdb.connect(host=config["mysql"]["host"], user=config["mysql"]["user"], passwd=config["mysql"]["pass"], db=config["mysql"]["name"], charset='utf8', use_unicode=True) 
		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute(query)
...
# def read_table_w_cfg_sensor_all(wr_id=''): 
# 	query = "SELECT * FROM " + config["table"]["rls_r"] + " WHERE wr_id = " + wr_id + " AND w_sensor_disable = 0 "
...
# 	query = "SELECT w_sensor_stop, w_sensor_reload, w_sensor_disable, w_alarm_disable FROM " + config["table"]["rls_r"] + " WHERE wr_id = " + wr_id
# 	try:
# 		conn = MySQLdb.connect(host=config["mysql"]["host"], user=config["mysql"]["user"], passwd=config["mysql"]["pass"], db=config["mysql"]["name"], charset='utf8', use_unicode=True) 
# 		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
# 		cursor.execute(query)
...
def set_reload_w_cfg_reload(wr_id=''): 
	query = "UPDATE " + config["table"]["rls_r"] + " SET w_sensor_reload = '0' WHERE wr_id = " + wr_id
	try:
		conn = MySQLdb.connect(host=config["mysql"]["host"], user=config["mysql"]["user"], passwd=config["mysql"]["pass"], db=config["mysql"]["name"], charset='utf8', use_unicode=True) 
		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute(query)
...
def check_scheduledDate(wr_id=''): # Optex Microwave
	...
	query = "SELECT COUNT(*) as cnt FROM " + ITS_sensor_blk_table + " WHERE wr_id = " + wr_id + " AND bo_table = '" + ECOS_table_RLS_R + "' AND startdate < '" + scheduledDate + "' AND enddate > '" + scheduledDate + "'"
	try:
		conn = MySQLdb.connect(host=config["mysql"]["host"], user=config["mysql"]["user"], passwd=config["mysql"]["pass"], db=config["mysql"]["name"], charset='utf8', use_unicode=True) 
		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute(query)
...
def check_scheduledWeek(wr_id='', weekNo=''): # Optex Microwave
	...
	query = "SELECT COUNT(*) as cnt FROM " + ITS_sensor_blk_table + " WHERE wr_id = " + wr_id + " AND w_week = 1 AND bo_table = '" + ECOS_table_RLS_R + "' AND startdate < '" + scheduledWeek + "' AND enddate > '" + scheduledWeek + "'"
	# print query
	try:
		conn = MySQLdb.connect(host=config["mysql"]["host"], user=config["mysql"]["user"], passwd=config["mysql"]["pass"], db=config["mysql"]["name"], charset='utf8', use_unicode=True) 
		cursor = conn.cursor(MySQLdb.cursors.DictCursor)
