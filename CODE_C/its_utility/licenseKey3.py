...
def main():
	...
	if byManual:
		...
		serialKey = input(G+"Serial No:\t"+W)
		# print("New IP Address(ex: 192.168.0.20)")
		# ipAddress = input(P+"New IP:\t"+W)
	else:
		keycode = autoKeycode
		serialKey = ""
		# ipAddress = ""
	...
	hash = sha256(serialKey + sha256(keycode.encode('utf-8')).hexdigest()).hexdigest()
	...
	while True:
		share["run"] = {} # 
		watch["fixed"]["run"] = ""
		selected = input("Select number(order) with space: ")
