...
read -p " Do you want upgrade Modules? " -n 1 upgrade;
echo    # (optional) move to a new line
if [[ $upgrade =~ [Yy]$ ]];
then
	cd ~;
	echo -e "$G >>> Modules Download and Upgrade. $X";
	sshpass -pits_iot scp pi@$itsSrvRemote:/home/pi/$downloadFolder/module/* ~;
	echo -e "$Y >>> Upgrade Python Modules $X";
	tar zxvf local.tgz;
	echo -e "$Y >>> Upgrade NodeJs Modules $X";
	tar zxvf node.tgz;
	rm local.tgz node.tgz;
fi
