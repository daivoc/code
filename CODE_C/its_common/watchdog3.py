...
def reportToWdSrv(ip, port, envData):
	...
	try: 
		...
		return sock.sendall(envData.encode('utf-8')) 
	except socket.error:
...
def get_diskSize():
	...
	dT = re.sub(' +',',',disk.strip().decode('utf-8')).split(',') #  15G  2.8G   11G  21%
	return dT
...
def main():
	...
	hash = sha256(serialKey.encode('utf8') + sha256(systemTitle.encode('utf8')).hexdigest().encode('utf8')).hexdigest()
	...
	data = {}
	data["fixed"] = {}
	data["fixed"]["lastStart"] = cmd_proc_Popen("uptime -s").strip().decode() # 마지막 부팅한 시각
	...
	while True:
		...
		topList = topInfo.decode('utf-8').split('\n')
