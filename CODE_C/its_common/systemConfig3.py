...
def run_demon_watchdog():
	...
	cmd = "python3 %s/watchdog3.pyc 2>&1 & " % (share["path"]["common"])
	# stdout=subprocess.PIPE, stderr=subprocess.PIPE 인경우 서브프로세서로 실행시 오류를 알수가 없다.
	p = subprocess.Popen(cmd, shell=True)
	return p
def kill_demon_watchdog():
	cmd = "pkill -9 -ef watchdog3.pyc 2>&1" 
	p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	time.sleep(1)
	return "kill_demon_watchdog3"

# 시스템 데몬 메니저 실행
def run_demon_userApi():
	print(kill_demon_userApi())
	cmd = "python3 %s/userApi3.pyc 2>&1 & " % (share["path"]["common"])
	# stdout=subprocess.PIPE, stderr=subprocess.PIPE 인경우 서브프로세서로 실행시 오류를 알수가 없다.
	p = subprocess.Popen(cmd, shell=True)
	return p
def kill_demon_userApi():
	cmd = "pkill -9 -ef userApi3.pyc 2>&1" 
	p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	time.sleep(1)
	return "kill_demon_userApi3"
...
def main():
	...
	for key in share["run"]:
		...
		if path and command:
			...
			cmd = "cd {0} && {1} {2} > /tmp/{3}.log".format(path,python,command,key)
...
if __name__ == "__main__":
	...
	its_iface = cmd_proc_Popen("ip addr | awk '/state UP/ {print $2}' | head -n 1 | sed 's/:$//' 2>/dev/null").strip().decode()
